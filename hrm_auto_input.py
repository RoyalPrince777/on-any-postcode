...
def run_once():
    ...
    # Load previous facts
    try:
        with open("hrm_last_facts.json", "r", encoding="utf-8") as f:
            previous_facts = json.load(f)
    except:
        previous_facts = []

    # Verify homepage facts
    for text in [
        "First member",
        "First business",
        "First creator",
    ...
    new_facts = [f for f in facts if f not in previous_facts]
    removed_facts = [f for f in previous_facts if f not in facts]
    unchanged_facts = [f for f in facts if f in previous_facts]

    report = f"""1. What OAP has learned

New facts:
{chr(10).join(new_facts) if new_facts else "None"}

Removed facts:
{chr(10).join(removed_facts) if removed_facts else "None"}

Unchanged facts:
{chr(10).join(unchanged_facts) if unchanged_facts else "None"}
...
"""
    ...
    # Save latest facts for comparison next run
    with open("hrm_last_facts.json", "w", encoding="utf-8") as f:
        json.dump(facts, f)

    record = {
        "created_at": datetime.now(UTC).isoformat(),
        "source": "oap_public_pages",
        "model": "python_change_detection_v0.1",
        "facts": facts,
        "new_facts": new_facts,
