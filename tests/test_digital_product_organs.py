...
def test_commerce_core_merchant_guard_requires_certification(monkeypatch):
    monkeypatch.setattr(certification, "identity_status", lambda _identity_id: {"merchant": False})
    with pytest.raises(PermissionError, match="certified_merchant_required"):
        product_core_views._require_certified_merchant("00000000-0000-0000-0000-000000000258")


def test_commerce_core_merchant_guard_fails_closed_when_store_unavailable(monkeypatch):
...
def test_commerce_core_seller_writes_call_certified_merchant_guard():
    ...
    storefront = source.split('def create_storefront():', 1)[1].split('def create_product():', 1)[0]
    product = source.split('def create_product():', 1)[1].split('def create_order():', 1)[0]
