...
def home():
    ...
    for p in products:
        ...
        <div class='card'>
        <b>{p['title']}</b><br>
        <span class='gold'>{p['category']}</span><br>
        <b>{p['price']}</b>
        """
        if p["image"]:
        ...
        content += f"""
        <p>{p['description']}</p>
        <a href='/product/{p['id']}'>Open Product</a>
        </div>
        """
    content += "</div></div>"
...
def admin():
    ...
    for p in products:
        content += f"""
        <div class='card'>
        <b>{p['title']}</b> by @{p['username']} — {p['status']}<br>
        <a href='/admin/product/{p['id']}/approved'>Approve</a>
        <a href='/admin/product/{p['id']}/rejected'>Reject</a>
        </div>
        """
    content += "</div>"
