...
def home():
    ...
    <div class='card'><h2>{count("sika_trust_records")}</h2><p>SIKA Trust Records</p></div>
    <div class='card'><h2>{sum_col("sika_trust_records","trust_points")}</h2><p>SIKA Trust Points</p></div>
    <div class='card'><h2>{count("awards")}</h2><p>Awards</p></div>
    <div class='card'><h2>{sum_col("awards","sika_points")}</h2><p>Award Points</p></div>
...
def my_world():
    ...
    <div class='card'><h2>{sum_col("sika_trust_records","trust_points","WHERE username=?",(u,))}</h2><p>My SIKA Points</p><a href='/sika_trust'>Open</a></div>
    <div class='card'><h2>{count("awards","WHERE username=?",(u,))}</h2><p>My Awards</p><a href='/awards'>Open</a></div>
    <div class='card'><h2>{sum_col("awards","sika_points","WHERE username=?",(u,))}</h2><p>My Award Points</p><a href='/awards'>Open</a></div>
...
def menu():
    ...
    <div class='grid'>
    <div class='card'><h2>🌍 My World</h2><p><a href='/my_world'>My World</a></p><p><a href='/family_tree'>Family Tree</a></p><p><a href='/affiliate_tree'>Affiliate Tree</a></p><p><a href='/sika_trust'>SIKA Trust</a></p><p><a href='/awards'>Awards</a></p></div>
...
def sika_trust():
    ...
    conn.close()
    content = """
    <div class='card hero'><h1>🪙 SIKA Trust Records</h1>
    <p class='green'>Not money. Not a bank. Not legal tender. Trust records only.</p></div>
    <div class='card warn'>SIKA records contribution, proof, trust and value. Financial features come later only after compliance readiness.</div>
    <div class='card'><form method='POST'>
    <select name='record_type'><option>Community Contribution</option><option>Business Value</option><option>Creator Value</option><option>Event Support</option><option>Affiliate Proof</option><option>Family Tree Legacy</option><option>Build Contribution</option><option>Trust Badge Proof</option><option>Award Proof</option></select>
    <input name='record_title' placeholder='Record title' required>
    <input name='linked_to' placeholder='Linked to member/business/event/build'>
    <textarea name='value_note' placeholder='What value was created?'></textarea>
    <textarea name='proof_note' placeholder='Proof note'></textarea>
    <input name='trust_points' placeholder='Trust points e.g. 10'>
    <button>Add SIKA Trust Record</button></form></div>
    """
    for r in rows:
        content += f"<div class='card'><b>{h(r['record_title'])}</b><br>{h(r['record_type'])} • {h(r['status'])}<p>{h(r['value_note'])}</p><p>{h(r['proof_note'])}</p><p class='gold'>SIKA Trust Points: {h(r['trust_points'])}</p><p class='small'>@{h(r['username'])} • {h(r['created_at'])}</p></div>"
...
def awards():
    if request.method == "POST":
        ...
        conn.execute("""INSERT INTO sika_trust_records(username,record_type,record_title,linked_to,value_note,proof_note,trust_points,status,created_at)
        VALUES(?,?,?,?,?,?,?,?,?)""",
        (user(), "Award Proof", request.form["award_name"], request.form["nominee_name"],
         request.form["reason"], request.form["proof_note"], points, "pending", now()))
    ...
    content = """
    <div class='card hero'><h1>🏆 OAP Awards</h1>
    <p class='green'>Awards recognize contribution. No pay-to-win. No fake authority.</p></div>
    <div class='card warn'>Awards must be proof-based and connected to community value, culture, service, creativity, business, or trust.</div>
    <div class='card'><form method='POST'>
    <input name='award_name' placeholder='Award name' required>
    <select name='award_type'>
      <option>Community Champion</option><option>Culture Keeper</option><option>Business Builder</option>
      <option>Creator Award</option><option>Event Organizer</option><option>Trust Builder</option>
      <option>SIKA Contributor</option><option>Heritage Guardian</option><option>Family Tree Legacy</option>
      <option>OAP Founder Award</option>
    </select>
    <input name='nominee_name' placeholder='Nominee name / member / business'>
    <input name='linked_record' placeholder='Linked SIKA / Family / Affiliate / Event record'>
    <textarea name='reason' placeholder='Why does this award matter?'></textarea>
    <textarea name='proof_note' placeholder='Proof note'></textarea>
    <input name='sika_points' placeholder='SIKA trust points e.g. 25'>
    <button>Add Award</button></form></div>
    """

    for r in rows:
        content += f"<div class='card'><b>{h(r['award_name'])}</b><br>{h(r['award_type'])} • {h(r['status'])}<p>Nominee: {h(r['nominee_name'])}</p><p>{h(r['reason'])}</p><p>{h(r['proof_note'])}</p><p class='gold'>Award Points: {h(r['sika_points'])}</p><p class='small'>@{h(r['username'])} • {h(r['created_at'])}</p></div>"
    return render(content)
...
def search():
    ...
    content = f"<div class='card hero'><h1>🔎 OAP Search</h1></div><div class='card'><form><input name='q' value='{h(q)}' placeholder='Search SIKA and Awards'><button>Search</button></form></div>"
    if not q:
        return render(content)
    like = f"%{q}%"
    conn = db()
    sika = conn.execute("""SELECT record_title, record_type, value_note, trust_points FROM sika_trust_records
    WHERE record_title LIKE ? OR record_type LIKE ? OR value_note LIKE ? OR proof_note LIKE ? LIMIT 50""", (like,like,like,like)).fetchall()
    awards_rows = conn.execute("""SELECT award_name, award_type, reason, sika_points FROM awards
    WHERE award_name LIKE ? OR award_type LIKE ? OR nominee_name LIKE ? OR reason LIKE ? LIMIT 50""", (like,like,like,like)).fetchall()
    conn.close()
    content += "<div class='card'><h2>Results</h2>"
    for r in sika:
        content += f"<div class='card'><b>SIKA: {h(r['record_title'])}</b><br>{h(r['record_type'])}<p>{h(r['value_note'])}</p><p class='gold'>{h(r['trust_points'])} points</p></div>"
    for r in awards_rows:
        content += f"<div class='card'><b>Award: {h(r['award_name'])}</b><br>{h(r['award_type'])}<p>{h(r['reason'])}</p><p class='gold'>{h(r['sika_points'])} points</p></div>"
    return render(content + "</div>")

@app.route("/explorer")
def explorer():
    return render("<div class='card hero'><h1>🧭 Explorer</h1></div><div class='grid'><div class='card'><h2>My World</h2><a href='/my_world'>Open</a></div><div class='card'><h2>SIKA Trust</h2><a href='/sika_trust'>Open</a></div><div class='card'><h2>Awards</h2><a href='/awards'>Open</a></div><div class='card'><h2>HRM</h2><a href='/hrm'>Open</a></div><div class='card'><h2>Sovereign Veto</h2><a href='/sovereign_veto'>Open</a></div></div>")
...
def command_center():
    ...
    <div class='grid'>
    <div class='card'><h2>{count("sika_trust_records")}</h2><p>SIKA Trust Records</p></div>
    <div class='card'><h2>{sum_col("sika_trust_records","trust_points")}</h2><p>Total SIKA Points</p></div>
    <div class='card'><h2>{count("awards")}</h2><p>Awards</p></div>
    <div class='card'><h2>{sum_col("awards","sika_points")}</h2><p>Award Points</p></div>
...
@app.route("/health")
def health():
    tables = ["users","audit_logs","family_tree","affiliate_tree","sika_trust_records","awards","council_logs","decision_logs","risk_logs","build_logs","sovereign_veto_logs"]
