...
SCHEMA_VERSION = "1.8_stability_core"
...
def init_db():
    ...
    cur.execute(
        ...
        (SCHEMA_VERSION, "OAP Stability Core loaded", now())
    )
...
def home():
    ...
    content += "<div class='card'><h2>🛡️ Active Stability Signals</h2>"
    if not risks:
        content += "<p class='green'>No open risks logged yet.</p>"
    for r in risks:
        content += f"<div class='card'><b>{r['title']}</b><br><span class='{r['severity'].lower()}'>{r['severity']}</span><p>{r['mitigation']}</p></div>"
    content += "</div>"
...
def register():
    ...
    <p class='small'>Create username admin if you want simple local admin access for this prototype.</p>
    </div>
    """)
...
def creators():
    ...
    content = "<div class='card'><h2>🎤 Creator Profiles</h2><p>OAP creator identity layer.</p>"
...
def market():
    ...
    <div class='card'>
    <h2>🛍️ OAP Market</h2>
    <p>OAP is the destination. External fulfilment/payment links are optional routes only.</p>
    """
...
def stability():
    ...
    if request.method == "POST":
        ...
        if form_type == "risk":
            conn.execute("""
            INSERT INTO risk_register(title,category,severity,status,mitigation,owner,created_at)
            VALUES(?,?,?,?,?,?,?)
            """,
            (
                request.form["title"],
        ...
        if form_type == "decision":
            conn.execute("""
            INSERT INTO decision_logs(title,decision,reason,approved_by,created_at)
            VALUES(?,?,?,?,?)
            """,
            (
                request.form["title"],
    ...
    content = """
    <div class='card'>
    <h2>🛡️ OAP Stability Core</h2>
    <p>Backup register • Risk register • Decision log • Schema version memory</p>
    <form method='POST' action='/backup_now'>
    <button>Create Database Backup</button>
    </form>
    </div>
    ...
    <textarea name='mitigation' placeholder='How do we reduce this risk?' required></textarea>
    ...
    <textarea name='decision' placeholder='What was decided?' required></textarea>
    <textarea name='reason' placeholder='Why?' required></textarea>
    ...
    content += "<div class='card'><h2>🧠 Decision Log</h2>"
    for d in decisions:
        content += f"<div class='card'><b>{d['title']}</b><p>{d['decision']}</p><p>{d['reason']}</p><span class='small'>Approved by {d['approved_by']} — {d['created_at']}</span></div>"
    content += "</div>"

    content += "<div class='card'><h2>💾 Backup Log</h2>"
    for b in backups:
        content += f"<div class='card'><b>{b['filename']}</b><p>{b['note']}</p><span class='small'>{b['created_by']} — {b['created_at']}</span></div>"
    content += "</div>"

    content += "<div class='card'><h2>🧱 Schema Memory</h2>"
    for s in schema:
...
def backup_now():
    ...
    if os.path.exists(DB):
        ...
        conn = db()
        conn.execute(
            "INSERT INTO backup_logs(filename,note,created_by,created_at) VALUES(?,?,?,?)",
            (filename, "Manual database backup created from Stability Core", session["user"], now())
        )
        conn.commit()
        conn.close()
...
def admin():
    ...
    backups = conn.execute("SELECT * FROM backup_logs ORDER BY id DESC LIMIT 10").fetchall()
    conn.close()
    ...
    content += "<div class='card'><h2>💾 Latest Backups</h2>"
    for b in backups:
        content += f"<div class='card'><b>{b['filename']}</b><br>{b['created_at']}</div>"
    content += "</div>"
