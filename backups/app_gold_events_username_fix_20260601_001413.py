...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS trust_badges(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        username TEXT,
    ...
    for table, cols in {
        "audit_logs": [("username","TEXT"),("member","TEXT"),("created_at","TEXT")],
        "culture_posts": [("username","TEXT"),("title","TEXT"),("category","TEXT"),("country","TEXT"),("body","TEXT"),("created_at","TEXT")],
        "artists": [("username","TEXT"),("artist_name","TEXT"),("genre","TEXT"),("country","TEXT"),("bio","TEXT"),("link","TEXT"),("status","TEXT DEFAULT 'pending'"),("created_at","TEXT")],
        "events": [("username","TEXT"),("title","TEXT"),("event_type","TEXT"),("postcode","TEXT"),("borough","TEXT"),("country","TEXT"),("venue","TEXT"),("event_date","TEXT"),("description","TEXT"),("status","TEXT DEFAULT 'pending'"),("created_at","TEXT")],
        "messages": [("sender","TEXT"),("recipient","TEXT"),("subject","TEXT"),("body","TEXT"),("status","TEXT DEFAULT 'unread'"),("created_at","TEXT")],
        "trust_badges": [("username","TEXT"),("requested_level","TEXT"),("proof","TEXT"),("status","TEXT DEFAULT 'pending'"),("created_at","TEXT")],
        "opportunities": [("username","TEXT"),("source","TEXT"),("description","TEXT"),("amount","REAL DEFAULT 0"),("currency","TEXT DEFAULT 'GBP'"),("status","TEXT DEFAULT 'recorded'"),("created_at","TEXT")],
        "payouts": [("username","TEXT"),("recipient_name","TEXT"),("reason","TEXT"),("amount","REAL DEFAULT 0"),("currency","TEXT DEFAULT 'GBP'"),("status","TEXT DEFAULT 'pending'"),("approved_by","TEXT"),("paid_date","TEXT"),("created_at","TEXT")],
        "approvals": [("approval_type","TEXT"),("record_id","INTEGER"),("status","TEXT DEFAULT 'pending'"),("reviewer","TEXT"),("notes","TEXT"),("created_at","TEXT")]
    }.items():
        for col, typ in cols:
...
def log(action, username="system"):
    conn = db()
    conn.execute("INSERT INTO audit_logs(action,username,member,created_at) VALUES(?,?,?,?)",
                 (action, username, username, now()))
    conn.commit()
    conn.close()
...
.top{background:#101010;padding:15px;border-bottom:1px solid #222;position:sticky;top:0}
...
.hero{text-align:center;padding:30px 10px}.green{color:#00dd99}.gold{color:#ffd166}
...
<a href="/">Home</a><a href="/privacy">Privacy</a><a href="/my_world">My World</a>
<a href="/command_center">Command Center</a><a href="/community">Community</a>
<a href="/sports">Sports</a><a href="/culture">Culture</a><a href="/music">Music</a>
<a href="/artists">Artists</a><a href="/events">Events</a><a href="/messages">Messenger</a>
<a href="/trust_badge">Trust Badge</a><a href="/opportunity_board">Opportunity Board</a>
<a href="/payouts">Payouts</a><a href="/approvals">Approvals</a>
<a href="/search">Search</a><a href="/explorer">Explorer</a><a href="/routes">Routes</a><a href="/admin">Admin</a>
...
def home():
    ...
    stats={
        ...
        "payouts":conn.execute("SELECT COALESCE(SUM(amount),0) c FROM payouts WHERE status='pending'").fetchone()["c"]
    }
    conn.close()
    return render(f"""
    <div class='card hero'><h1>🌍 ON ANY POSTCODE 👑</h1>
    <p class='green'>Join OAP. Enter My World. Build trust through contribution.</p>
    <p>Every member has a World.</p></div>
    <div class='grid'>
    ...
    <div class='card'><h2>£{stats['payouts']:.2f}</h2><p>Pending Payouts</p></div>
    </div>
    """)
...
@app.route("/welcome")
def welcome():
    return render("<div class='card hero'><h1>🌍 Your World Has Been Created</h1><p class='green'>Welcome to OAP.</p><a href='/enter'>Enter My World</a></div>")
...
def enter():
    ...
    </form><p>Founder: N24-7 / 2525</p></div>
    """)
...
def privacy():
    return render("""
    <div class='card hero'><h1>🛡️ OAP Privacy & Trust Promise</h1>
    <p class='green'>Human-first. Private-first. Community-first.</p></div>
    <div class='card'><h2>Your OAP Rights</h2>
    <p>✅ Your data belongs to you.</p><p>✅ Your profile belongs to you.</p>
    <p>✅ Your content belongs to you.</p><p>✅ Your messages belong to you.</p>
    <p>✅ Your culture belongs to you.</p><p>✅ Your business belongs to you.</p></div>
...
def my_world():
    ...
    <div class='card'><h2>⭐</h2><p>Trust Badge</p><a href='/trust_badge'>Open</a></div>
    </div>
    """)
...
def culture():
    if request.method=="POST":
        conn=db()
        conn.execute("INSERT INTO culture_posts(username,title,category,country,body,created_at) VALUES(?,?,?,?,?,?)",
                     (user(),request.form["title"],request.form["category"],request.form["country"],request.form["body"],now()))
        conn.commit(); conn.close()
        log("Culture created", user())
        return redirect("/culture")
    conn=db()
    rows=conn.execute("SELECT * FROM culture_posts ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
    ...
    for r in rows:
        content+=f"<div class='card'><b>{h(r['title'])}</b><br>{h(r['category'])} • {h(r['country'])}<p>{h(r['body'])}</p></div>"
...
def artists():
    if request.method=="POST":
        conn=db()
        conn.execute("INSERT INTO artists(username,artist_name,genre,country,bio,link,status,created_at) VALUES(?,?,?,?,?,?,?,?)",
                     (user(),request.form["artist_name"],request.form["genre"],request.form["country"],request.form["bio"],request.form["link"],"pending",now()))
        conn.commit(); conn.close()
        return redirect("/artists")
    conn=db()
    rows=conn.execute("SELECT * FROM artists ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
...
def events():
    if request.method=="POST":
        conn=db()
        conn.execute("INSERT INTO events(username,title,event_type,postcode,borough,country,venue,event_date,description,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                     (user(),request.form["title"],request.form["event_type"],request.form["postcode"],request.form["borough"],request.form["country"],request.form["venue"],request.form["event_date"],request.form["description"],"pending",now()))
        conn.commit(); conn.close()
        return redirect("/events")
    conn=db()
    rows=conn.execute("SELECT * FROM events ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
    ...
    for r in rows:
        content+=f"<div class='card'><b>{h(r['title'])}</b><br>{h(r['event_type'])} • {h(r['postcode'])} • {h(r['status'])}<p>{h(r['description'])}</p></div>"
    return render(content)
...
def messages():
    if request.method=="POST":
        ...
        conn.execute("INSERT INTO messages(sender,recipient,subject,body,status,created_at) VALUES(?,?,?,?,?,?)",
                     (user(),request.form["recipient"],request.form["subject"],request.form["body"],"unread",now()))
        conn.commit(); conn.close()
        return redirect("/messages")
    u=user()
    conn=db()
    rows=conn.execute("SELECT * FROM messages WHERE sender=? OR recipient=? OR recipient='admin' ORDER BY id DESC LIMIT 80",(u,u)).fetchall()
    conn.close()
    ...
    for r in rows:
        content+=f"<div class='card'><b>{h(r['subject'])}</b><br>{h(r['sender'])} → {h(r['recipient'])}<p>{h(r['body'])}</p></div>"
    return render(content)
...
def trust_badge():
    if request.method=="POST":
        conn=db()
        conn.execute("INSERT INTO trust_badges(username,requested_level,proof,status,created_at) VALUES(?,?,?,?,?)",
                     (user(),request.form["requested_level"],request.form["proof"],"pending",now()))
        conn.commit(); conn.close()
        return redirect("/trust_badge")
    conn=db()
    rows=conn.execute("SELECT * FROM trust_badges ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
...
def opportunity():
    if request.method=="POST":
        conn=db()
        conn.execute("INSERT INTO opportunities(username,source,description,amount,currency,status,created_at) VALUES(?,?,?,?,?,?,?)",
                     (request.form.get("username") or user(),request.form["source"],request.form["description"],float(request.form.get("amount") or 0),request.form["currency"],"recorded",now()))
        conn.commit(); conn.close()
        return redirect("/opportunity_board")
    conn=db()
    rows=conn.execute("SELECT * FROM opportunities ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
    ...
    for r in rows:
        content+=f"<div class='card'><b>{h(r['source'])}</b><br>{h(r['currency'])} {h(r['amount'])} • @{h(r['username'])}<p>{h(r['description'])}</p></div>"
    return render(content)
...
def payouts():
    if request.method=="POST":
        ...
        cur.execute("INSERT INTO payouts(username,recipient_name,reason,amount,currency,status,approved_by,paid_date,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
                    (request.form.get("username") or user(),request.form["recipient_name"],request.form["reason"],float(request.form.get("amount") or 0),request.form["currency"],"pending","","",now()))
        pid=cur.lastrowid
        cur.execute("INSERT INTO approvals(approval_type,record_id,status,reviewer,notes,created_at) VALUES(?,?,?,?,?,?)",
                    ("payout",pid,"pending","","Payout requires human approval.",now()))
        conn.commit(); conn.close()
        return redirect("/payouts")
    conn=db()
    rows=conn.execute("SELECT * FROM payouts ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
    ...
    for r in rows:
        content+=f"<div class='card'><b>{h(r['recipient_name'])}</b><br>{h(r['currency'])} {h(r['amount'])} • {h(r['status'])}<p>{h(r['reason'])}</p></div>"
    return render(content)

@app.route("/approvals")
def approvals():
    conn=db()
    rows=conn.execute("SELECT * FROM approvals ORDER BY id DESC LIMIT 80").fetchall()
    conn.close()
    content="<div class='card hero'><h1>✅ Approvals</h1></div>"
    for r in rows:
        content+=f"<div class='card'><b>{h(r['approval_type'])} #{h(r['record_id'])}</b><br>{h(r['status'])}<p>{h(r['notes'])}</p><a href='/approval/{r['id']}/approved'>Approve</a><a href='/approval/{r['id']}/rejected'>Reject</a><a href='/approval/{r['id']}/paid'>Mark Paid</a></div>"
    return render(content)
...
def approval_action(id,status):
    ...
    reviewer=user()
    conn=db()
    a=conn.execute("SELECT * FROM approvals WHERE id=?",(id,)).fetchone()
    if a:
        ...
        if a["approval_type"]=="payout":
            if status=="paid":
                conn.execute("UPDATE payouts SET status=?, approved_by=?, paid_date=? WHERE id=?",("paid",reviewer,now(),a["record_id"]))
            else:
                conn.execute("UPDATE payouts SET status=?, approved_by=? WHERE id=?",(status,reviewer,a["record_id"]))
        conn.commit()
    conn.close()
    return redirect("/approvals")
...
def search():
    ...
    content=f"<div class='card hero'><h1>🔎 OAP Search</h1><p class='green'>OAP-first search. Not Google.</p></div><div class='card'><form method='GET'><input name='q' value='{h(q)}' placeholder='Search culture, artists, events...'><button>Search</button></form></div>"
    if not q:
        return render(content)
    like=f"%{q}%"; results=[]; conn=db()
    for r in conn.execute("SELECT title,category,body FROM culture_posts WHERE title LIKE ? OR body LIKE ? LIMIT 20",(like,like)):
        results.append(("Culture",r["title"],r["category"],r["body"]))
    for r in conn.execute("SELECT artist_name,genre,bio FROM artists WHERE artist_name LIKE ? OR genre LIKE ? OR bio LIKE ? LIMIT 20",(like,like,like)):
        results.append(("Artist",r["artist_name"],r["genre"],r["bio"]))
    for r in conn.execute("SELECT title,event_type,description FROM events WHERE title LIKE ? OR event_type LIKE ? OR description LIKE ? LIMIT 20",(like,like,like)):
        results.append(("Event",r["title"],r["event_type"],r["description"]))
    conn.close()
...
@app.route("/explorer")
def explorer():
    return render("<div class='card hero'><h1>🧭 Explorer</h1><p class='green'>Explore OAP by community, culture, events, artists and opportunity.</p></div><div class='grid'><div class='card'><h2>🌍 Community</h2><a href='/community'>Open</a></div><div class='card'><h2>🎵 Culture</h2><a href='/culture'>Open</a></div><div class='card'><h2>🎤 Artists</h2><a href='/artists'>Open</a></div><div class='card'><h2>📅 Events</h2><a href='/events'>Open</a></div><div class='card'><h2>💰 Opportunity</h2><a href='/opportunity_board'>Open</a></div></div>")
...
def command_center():
    ...
    approvals_count=conn.execute("SELECT COUNT(*) c FROM approvals WHERE status='pending'").fetchone()["c"]
    members=conn.execute("SELECT COUNT(*) c FROM users").fetchone()["c"]
    conn.close()
    return render(f"<div class='card hero'><h1>🎯 Command Center</h1></div><div class='grid'><div class='card'><h2>{members}</h2><p>Members</p></div><div class='card'><h2>£{total:.2f}</h2><p>Opportunity Board</p></div><div class='card'><h2>£{pending:.2f}</h2><p>Pending Payouts</p></div><div class='card'><h2>£{paid:.2f}</h2><p>Paid Payouts</p></div><div class='card'><h2>{approvals_count}</h2><p>Pending Approvals</p></div></div>")
...
def admin():
    conn=db()
    users=conn.execute("SELECT * FROM users ORDER BY id DESC LIMIT 80").fetchall()
    ...
    for m in users:
        content+=f"<div class='card'>@{h(m['username'])} — {h(m['verification_level'])}<br>{h(m['postcode'])} → {h(m['country'])}</div>"
