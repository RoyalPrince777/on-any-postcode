...
app = Flask(__name__)
app.secret_key = "change_this_oap_bee_coordination_secret"
DB = "oap_world.db"
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS bee_tasks(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT,
        mission_type TEXT,
        assigned_to TEXT,
...
<title>OAP Bee Coordination</title>
<style>
...
.bee{background:#1f1a08;border-color:#f4c542}
</style>
</head>
<body>
<header>
<h1>🐝 OAP World — Bee Coordination</h1>
<nav>
<a href="/">Home</a>
<a href="/explorer">Explorer</a>
<a href="/dashboard">Dashboard</a>
<a href="/bee">Bee Board</a>
<a href="/add-task">Add Task</a>
<a href="/add-volunteer">Add Volunteer</a>
...
def home():
    ...
    <div class="grid">
      ...
      <div class="card bee"><h3>🐝 Bee Coordination</h3><p>Tasks, missions, volunteers, community action.</p></div>
...
def bee():
    conn = db()
    tasks = conn.execute("SELECT * FROM bee_tasks ORDER BY id DESC LIMIT 25").fetchall()
    volunteers = conn.execute("SELECT * FROM volunteers ORDER BY id DESC LIMIT 10").fetchall()

    task_count = count_table(conn, "bee_tasks")
    volunteer_count = count_table(conn, "volunteers")
    open_tasks = conn.execute("SELECT COUNT(*) FROM bee_tasks WHERE status!='Done'").fetchone()[0]
    done_tasks = conn.execute("SELECT COUNT(*) FROM bee_tasks WHERE status='Done'").fetchone()[0]
    conn.close()

    html = f"""
    <div class="card bee">
    <h2>🐝 Bee Coordination Board</h2>
    ...
    <div class="grid">
      <div class="card"><div class="big">{task_count}</div><p>Total Tasks / Missions</p></div>
      <div class="card"><div class="big">{open_tasks}</div><p>Open Actions</p></div>
      <div class="card"><div class="big">{done_tasks}</div><p>Completed Actions</p></div>
      <div class="card"><div class="big">{volunteer_count}</div><p>Volunteers</p></div>
    </div>
    </div>
    """

    html += "<div class='card'><h2>📌 Latest Tasks / Missions</h2>"
    if not tasks:
        html += "<p>No tasks yet. Add your first action.</p>"
    for t in tasks:
        html += f"""
        <div class="card">
        <h3>#{t['id']} {t['title']}</h3>
        <p><span class="badge">{t['mission_type']}</span> <span class="badge">{t['priority']}</span> <span class="badge">{t['status']}</span></p>
        <p><b>Assigned:</b> {t['assigned_to']} | <b>Postcode:</b> {t['postcode']} | <b>Country:</b> {t['country']}</p>
        <p>{t['action_note']}</p>
        <p><a class="badge" href="/task-done/{t['id']}">Mark Done</a></p>
        <p class="small">{t['created_at']}</p>
        </div>
        """
    html += "</div>"

    html += "<div class='card'><h2>🤝 Volunteers</h2>"
    if not volunteers:
        html += "<p>No volunteers yet.</p>"
    for v in volunteers:
        html += f"""
        <p><span class="badge">{v['name']}</span> {v['skill']} — {v['postcode']} / {v['country']} — {v['availability']} — {v['status']}</p>
        """
    html += "</div>"
...
def add_task():
    if request.method == "POST":
        conn = db()
        conn.execute("""INSERT INTO bee_tasks
        (title, mission_type, assigned_to, postcode, country, priority, status, action_note, created_at)
        VALUES (?,?,?,?,?,?,?,?,?)""", (
            request.form["title"],
            request.form["mission_type"],
            request.form["assigned_to"],
            request.form["postcode"],
            request.form["country"],
            request.form["priority"],
            request.form["status"],
            request.form["action_note"],
            now()
        ))
        conn.commit()
        conn.close()
    ...
    return page("""
    <div class="card bee"><h2>🐝 Add Bee Task / Mission</h2>
    <form method="post">
    <input name="title" placeholder="Task title e.g. Invite first business" required>
    <select name="mission_type">
      <option>First Member</option>
      <option>First Business</option>
      <option>First Event</option>
      <option>First Revenue</option>
      <option>News Content</option>
      <option>Creator Outreach</option>
    ...
    <textarea name="action_note" placeholder="Action note / proof / next step"></textarea>
    <button>Save Bee Task</button>
    </form></div>
    """)
...
def task_done(task_id):
    conn = db()
    conn.execute("UPDATE bee_tasks SET status='Done' WHERE id=?", (task_id,))
    conn.commit()
    conn.close()
    return redirect("/bee")
...
def add_volunteer():
    if request.method == "POST":
        ...
        (name, skill, postcode, country, availability, status, created_at)
        VALUES (?,?,?,?,?,?,?)""", (
            request.form["name"],
            request.form["skill"],
            request.form["postcode"],
    ...
    return page("""
    <div class="card"><h2>🤝 Add Volunteer / Helper</h2>
    <form method="post">
    <input name="name" placeholder="Name / username" required>
    <input name="skill" placeholder="Skill e.g. design, events, business outreach">
    <input name="postcode" placeholder="Postcode">
    <input name="country" placeholder="Country">
    <input name="availability" placeholder="Availability e.g. weekends, evenings">
    <select name="status">
...
def dashboard():
    ...
    metrics = {
        ...
        "Bee Tasks": count_table(conn, "bee_tasks"),
        "Volunteers": count_table(conn, "volunteers")
    }
