...
def home():
    ...
    missions = conn.execute("SELECT * FROM missions ORDER BY id DESC LIMIT 5").fetchall()
    ...
    joke_cards = "".join([f"<div class='card'><h2>😂 {j['category']}</h2><p>{j['joke']}</p><small>By {j['name']} · {j['created_at']}</small></div>" for j in jokes])
    ...
    {joke_cards or "<div class='card'>No laughs yet. Add the first joke.</div>"}
...
def fun():
    ...
    <h2>😂 {h['category']}</h2>
    <p>{h['joke']}</p>
    <small>By {h['name']} · {h['created_at']}</small>
    </div>
    """ for h in rows])
