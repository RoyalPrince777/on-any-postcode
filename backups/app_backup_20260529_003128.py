...
def messages():
    ...
    content = "<div class='card'><h2>💬 Messenger</h2><p>Private OAP user messages.</p></div>"
    content += "<div class='card'><h3>Send New Message</h3>"
    for u in users:
    ...
    for m in inbox:
        ...
        <div class='card'>
            <b>{direction}: @{other}</b>
            <p>{m['body']}</p>
            <span class='small'>{m['created_at']}</span><br>
            <a href='/message/{other}'>Open Thread</a>
        </div>
        """
    content += "</div>"
    return render_template_string(BASE, content=content)
