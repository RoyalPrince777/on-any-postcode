...
def count(table, where="", params=()):
    conn = db()
    try:
        return conn.execute(f"SELECT COUNT(*) c FROM {table} {where}", params).fetchone()["c"]
    except Exception:
        return 0
    finally:
        conn.close()
...
def sum_col(table, col, where="", params=()):
    conn = db()
    try:
        return conn.execute(f"SELECT COALESCE(SUM({col}),0) s FROM {table} {where}", params).fetchone()["s"]
    except Exception:
        return 0
    finally:
        conn.close()
...
<div class="nav">
<a href="/">Home</a><a href="/my_world">My World</a><a href="/prince">Prince</a>
<a href="/megaverse">Megaverse</a><a href="/search">Search</a><a href="/command_center">Command Center</a>
<a href="/sika_trust">SIKA Trust</a><a href="/awards">Awards</a><a href="/hrm">HRM</a><a href="/menu">Menu</a>
...
def home():
    ...
    <p>Royalty protects legacy. HRM records truth. SIKA records trust. Awards recognize contribution.</p>
...
def prince():
    if request.method == "POST":
        ...
        VALUES(?,?,?,?,?,?,?,?,?)""",
        (user(), request.form["archive_title"], request.form["archive_type"], request.form["heritage_link"],
         request.form["stewardship_note"], request.form["legacy_note"], request.form["visibility"], "active", now()))
        conn.commit()
        conn.close()
        log("Royal Archive record added", user())
        return redirect("/prince")

    conn = db()
    rows = conn.execute("SELECT * FROM royal_archive ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()

    content = """
    <div class='card hero'><h1>👑 Prince of KORADASO</h1>
    <p class='green'>Royalty means service. Stewardship. Legacy. Culture. Protection.</p></div>
    <div class='card warn'>This section is a heritage and stewardship layer. It is not government authority, banking authority, legal tender, or control over people.</div>
    <div class='grid'>
      <div class='card'><h2>🌳 Family Tree</h2><p>Roots and relationships.</p><a href='/family_tree'>Open</a></div>
      <div class='card'><h2>🪙 SIKA Trust</h2><p>Contribution and proof.</p><a href='/sika_trust'>Open</a></div>
      <div class='card'><h2>🏆 Awards</h2><p>Recognition and legacy.</p><a href='/awards'>Open</a></div>
      <div class='card'><h2>🧠 HRM</h2><p>Record keeper.</p><a href='/hrm'>Open</a></div>
    </div>
    <div class='card'><form method='POST'>
    <input name='archive_title' placeholder='Royal archive title' required>
    <select name='archive_type'>
      <option>Heritage Record</option><option>Stewardship Note</option><option>Legacy Record</option>
      <option>Culture Protection</option><option>Family Record</option><option>Award Record</option>
    </select>
    <input name='heritage_link' placeholder='Linked Family / Culture / Award / SIKA record'>
    <textarea name='stewardship_note' placeholder='Stewardship note'></textarea>
    <textarea name='legacy_note' placeholder='Legacy note'></textarea>
    <select name='visibility'><option>private</option><option>public</option></select>
    <button>Add Royal Archive Record</button></form></div>
    """
    for r in rows:
        content += f"<div class='card'><b>{h(r['archive_title'])}</b><br>{h(r['archive_type'])} • {h(r['visibility'])}<p>{h(r['stewardship_note'])}</p><p>{h(r['legacy_note'])}</p><p class='small'>@{h(r['username'])} • {h(r['created_at'])}</p></div>"
    return render(content)

@app.route("/megaverse")
def megaverse():
    return render(f"""
    <div class='card hero'><h1>🌌 Sovereign Megaverse Dashboard</h1>
    <p class='green'>Human-first intelligence from real records.</p></div>
    <div class='grid'>
    <div class='card'><h2>85% 🟢</h2><p>Personal HRM Foundation</p></div>
    <div class='card'><h2>95% 🟢</h2><p>Sovereign Council</p></div>
    <div class='card'><h2>45% 🟡</h2><p>Royal Stewardship</p></div>
    <div class='card'><h2>70% 🟡</h2><p>Trust Intelligence</p></div>
    <div class='card'><h2>25% 🔴</h2><p>Commerce Intelligence</p></div>
    <div class='card'><h2>15% 🔴</h2><p>Distribution Intelligence</p></div>
    </div>
    <div class='card'><h2>Live Records</h2>
    <p>Family Tree: {count("family_tree")}</p>
    <p>SIKA Trust: {count("sika_trust_records")}</p>
    <p>Awards: {count("awards")}</p>
    <p>Royal Archive: {count("royal_archive")}</p>
    <p>Decision Logs: {count("decision_logs")}</p>
    <p>Risk Logs: {count("risk_logs")}</p>
    </div>
    """)
...
def menu():
    ...
    <div class='grid'>
    <div class='card'><h2>👑 Royal</h2><p><a href='/prince'>Prince Section</a></p><p><a href='/megaverse'>Megaverse Dashboard</a></p></div>
    <div class='card'><h2>🌍 My World</h2><p><a href='/my_world'>My World</a></p><p><a href='/family_tree'>Family Tree</a></p><p><a href='/sika_trust'>SIKA Trust</a></p><p><a href='/awards'>Awards</a></p></div>
    <div class='card'><h2>🧠 HRM</h2><p><a href='/hrm'>HRM</a></p><p><a href='/decision_logs'>Decision Logs</a></p><p><a href='/risk_logs'>Risk Logs</a></p><p><a href='/sovereign_veto'>Sovereign Veto</a></p></div>
    <div class='card'><h2>System</h2><p><a href='/privacy'>Privacy</a></p><p><a href='/health'>Health</a></p><p><a href='/routes'>Routes</a></p></div>
    </div>
    """)
...
def sika_trust():
    if request.method == "POST":
        ...
        VALUES(?,?,?,?,?,?,?,?,?)""",
        (user(), request.form["record_type"], request.form["record_title"], request.form["linked_to"],
         request.form["value_note"], request.form["proof_note"], points, "pending", now()))
        conn.commit()
        conn.close()
        log("SIKA Trust record added", user())
        return redirect("/sika_trust")
    conn = db()
    rows = conn.execute("SELECT * FROM sika_trust_records ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()
    content = """
    <div class='card hero'><h1>🪙 SIKA Trust Records</h1><p class='green'>Not money. Not a bank. Trust records only.</p></div>
    <div class='card'><form method='POST'>
    <select name='record_type'><option>Community Contribution</option><option>Business Value</option><option>Creator Value</option><option>Award Proof</option><option>Royal Archive</option></select>
    <input name='record_title' placeholder='Record title' required>
    <input name='linked_to' placeholder='Linked to'>
    <textarea name='value_note' placeholder='Value note'></textarea>
    <textarea name='proof_note' placeholder='Proof note'></textarea>
    <input name='trust_points' placeholder='Trust points'>
    <button>Add SIKA Trust Record</button></form></div>
    """
    for r in rows:
        content += f"<div class='card'><b>{h(r['record_title'])}</b><br>{h(r['record_type'])}<p>{h(r['value_note'])}</p><p class='gold'>{h(r['trust_points'])} points</p></div>"
    return render(content)
...
def awards():
    if request.method == "POST":
        ...
        VALUES(?,?,?,?,?,?,?,?,?,?)""",
        (user(), request.form["award_name"], request.form["award_type"], request.form["nominee_name"],
         request.form["linked_record"], request.form["reason"], request.form["proof_note"], points, "pending", now()))
        conn.commit()
        conn.close()
        log("Award record added", user())
        return redirect("/awards")
    conn = db()
    rows = conn.execute("SELECT * FROM awards ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()
    content = """
    <div class='card hero'><h1>🏆 OAP Awards</h1><p class='green'>Awards recognize contribution. No pay-to-win.</p></div>
    <div class='card'><form method='POST'>
    <input name='award_name' placeholder='Award name' required>
    <select name='award_type'><option>Community Champion</option><option>Culture Keeper</option><option>Business Builder</option><option>Creator Award</option><option>Trust Builder</option><option>Heritage Guardian</option></select>
    <input name='nominee_name' placeholder='Nominee'>
    <input name='linked_record' placeholder='Linked record'>
    <textarea name='reason' placeholder='Reason'></textarea>
    <textarea name='proof_note' placeholder='Proof note'></textarea>
    <input name='sika_points' placeholder='Award points'>
    <button>Add Award</button></form></div>
    """
    for r in rows:
        content += f"<div class='card'><b>{h(r['award_name'])}</b><br>{h(r['award_type'])} • {h(r['status'])}<p>{h(r['reason'])}</p><p class='gold'>{h(r['sika_points'])} points</p></div>"
    return render(content)
...
def council_logs():
    return log_page("council_logs", "👑 Council Logs", {"council_role":"Council role", "signal":"Signal", "summary":"Summary", "recommendation":"Recommendation", "status":"Status"})
...
def affiliate_tree():
    return render("<div class='card hero'><h1>🌿 Affiliate Tree</h1><p class='green'>Rewards follow real value, not recruitment.</p></div>")
...
@app.route("/explorer")
def explorer():
    return render("<div class='card hero'><h1>🧭 Explorer</h1></div><div class='grid'><div class='card'><h2>Prince</h2><a href='/prince'>Open</a></div><div class='card'><h2>Megaverse</h2><a href='/megaverse'>Open</a></div><div class='card'><h2>SIKA Trust</h2><a href='/sika_trust'>Open</a></div><div class='card'><h2>Awards</h2><a href='/awards'>Open</a></div></div>")
...
@app.route("/health")
def health():
    tables = ["users","audit_logs","family_tree","affiliate_tree","sika_trust_records","awards","royal_archive","council_logs","decision_logs","risk_logs","build_logs","sovereign_veto_logs"]
