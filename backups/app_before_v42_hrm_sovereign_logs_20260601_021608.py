...
def home():
    ...
    <p class='green'>Clarity. Speed. Accuracy.</p>
...
def menu():
    return render("""
    <div class='card hero'><h1>☰ OAP Menu</h1><p class='green'>Tidy menu. Same power, less clutter.</p></div>
    <div class='grid'>
      <div class='card'><h2>🌍 Public World</h2>
      <p><a href='/community'>Community</a></p><p><a href='/sports'>Sports</a></p><p><a href='/culture'>Culture</a></p><p><a href='/music'>Music</a></p><p><a href='/artists'>Artists</a></p><p><a href='/events'>Events</a></p></div>
      <div class='card'><h2>🌍 My World</h2>
      <p><a href='/my_world'>My World</a></p><p><a href='/messages'>Messenger</a></p><p><a href='/trust_badge'>Trust Badge</a></p><p><a href='/family_tree'>Family Tree</a></p><p><a href='/affiliate_tree'>Affiliate Tree</a></p><p><a href='/payouts'>Payouts</a></p></div>
      ...
      <p><a href='/privacy'>Privacy & Trust</a></p><p><a href='/affiliate_rules'>Affiliate Rules</a></p><p><a href='/command_center'>Command Center</a></p><p><a href='/routes'>Routes</a></p><p><a href='/health'>Health Check</a></p><p><a href='/admin'>Admin</a></p></div>
    </div>
    """)
...
def my_world():
    ...
    family=conn.execute("SELECT COUNT(*) c FROM family_tree WHERE username=?", (u,)).fetchone()["c"]
    affiliate=conn.execute("SELECT COUNT(*) c FROM affiliate_tree WHERE username=?", (u,)).fetchone()["c"]
    ...
    <div class='card'><h2>{family}</h2><p>My Family Tree</p><a href='/family_tree'>Open</a></div>
    <div class='card'><h2>{affiliate}</h2><p>My Affiliate Tree</p><a href='/affiliate_tree'>Open</a></div>
...
def family_tree():
    if request.method=="POST":
        conn=db()
        conn.execute("""INSERT INTO family_tree(username,node_name,node_type,connection_to,relationship,heritage_note,contribution_note,visibility,status,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?)""",
        (user(),request.form["node_name"],request.form["node_type"],request.form["connection_to"],request.form["relationship"],
         request.form["heritage_note"],request.form["contribution_note"],request.form["visibility"],"active",now()))
        conn.commit(); conn.close(); log("Family Tree record added", user())
        return redirect("/family_tree")
    conn=db()
    rows=conn.execute("SELECT * FROM family_tree WHERE username=? OR visibility='public' ORDER BY id DESC LIMIT 100",(user(),)).fetchall()
    conn.close()
    content="""
    <div class='card hero'><h1>🌳 Family Tree</h1>
    <p class='green'>Roots are heritage. Branches are relationships. Leaves are communities. Fruit is contribution.</p></div>
    <div class='card warn'>Private-first. This is not a recruitment system.</div>
    <div class='card'><form method='POST'>
    <input name='node_name' placeholder='Name / group / community / mentor' required>
    <select name='node_type'><option>Family</option><option>Heritage</option><option>Mentor</option><option>Community</option><option>Creator</option><option>Business</option><option>Legacy</option></select>
    <input name='connection_to' placeholder='Connected to'>
    <input name='relationship' placeholder='Relationship / role'>
    <textarea name='heritage_note' placeholder='Heritage note'></textarea>
    <textarea name='contribution_note' placeholder='Contribution / trust note'></textarea>
    <select name='visibility'><option>private</option><option>public</option></select>
    <button>Add Family Tree Record</button></form></div>
    """
    for r in rows:
        content+=f"<div class='card'><b>{h(r['node_name'])}</b><br>{h(r['node_type'])} → {h(r['connection_to'])}<br><span class='green'>{h(r['relationship'])}</span><p>{h(r['heritage_note'])}</p><p>{h(r['contribution_note'])}</p></div>"
    return render(content)

@app.route("/affiliate_rules")
def affiliate_rules():
    return render("""
    <div class='card hero'><h1>🌿 Affiliate Tree Rules</h1><p class='green'>The tree is not a pyramid. The tree is proof of trust.</p></div>
    <div class='card'>
    <p>✅ Direct commission only.</p>
    <p>✅ Reward real sales and real value only.</p>
    <p>✅ No deep commission chains.</p>
    <p>✅ No fake income promises.</p>
    <p>✅ No pressure, no debt, no recruitment games.</p>
    <p>✅ Youth version must use badges, cards, learning points, not cash-pressure selling.</p>
    <p>✅ Human approval before payout.</p>
    </div>
    """)

@app.route("/affiliate_tree", methods=["GET","POST"])
def affiliate_tree():
    if request.method=="POST":
        sale_value = float(request.form.get("sale_value") or 0)
        commission_value = float(request.form.get("commission_value") or 0)
        conn=db()
        conn.execute("""INSERT INTO affiliate_tree(username,affiliate_name,affiliate_type,referred_business,sale_value,commission_value,currency,proof_note,status,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?)""",
        (user(),request.form["affiliate_name"],request.form["affiliate_type"],request.form["referred_business"],
         sale_value,commission_value,request.form["currency"],request.form["proof_note"],"pending",now()))
        cur = conn.cursor()
        cur.execute("""INSERT INTO approvals(approval_type,record_id,status,reviewer,notes,created_at)
        VALUES(?,?,?,?,?,?)""", ("affiliate_commission", cur.lastrowid, "pending", "", "Affiliate commission requires human approval.", now()))
        conn.commit(); conn.close(); log("Affiliate Tree record added", user())
        return redirect("/affiliate_tree")
    conn=db()
    rows=conn.execute("SELECT * FROM affiliate_tree WHERE username=? ORDER BY id DESC LIMIT 100",(user(),)).fetchall()
    conn.close()
    content="""
    <div class='card hero'><h1>🌿 Affiliate Tree</h1>
    <p class='green'>Rewards follow real value, not recruitment.</p></div>
    <div class='card warn'>Direct commission only. No pyramid. No pressure. No fake income promises.</div>
    <div class='card'><form method='POST'>
    <input name='affiliate_name' placeholder='Affiliate / ambassador name' required>
    <select name='affiliate_type'><option>Business Lead</option><option>Creator Lead</option><option>Event Lead</option><option>Product Sale</option><option>Promotion Sale</option><option>Mentor Support</option></select>
    <input name='referred_business' placeholder='Business / creator / event referred'>
    <input name='sale_value' placeholder='Sale value e.g. 50'>
    <input name='commission_value' placeholder='Commission value e.g. 10'>
    <select name='currency'><option>GBP</option><option>GHS</option><option>EUR</option><option>SIKA</option></select>
    <textarea name='proof_note' placeholder='Proof note: who paid, what value was created, what was agreed'></textarea>
    <button>Add Affiliate Record</button></form></div>
    """
    for r in rows:
        content+=f"<div class='card'><b>{h(r['affiliate_name'])}</b><br>{h(r['affiliate_type'])} → {h(r['referred_business'])}<p>Sale: {h(r['currency'])} {h(r['sale_value'])} | Commission: {h(r['currency'])} {h(r['commission_value'])}</p><p>{h(r['proof_note'])}</p><p class='small'>Status: {h(r['status'])} • {h(r['created_at'])}</p></div>"
    return render(content)
...
@app.route("/payouts")
def payouts():
    return render("<div class='card hero'><h1>💸 Payouts</h1><p class='green'>Manual only. Human approval before money moves.</p></div>")
...
def approvals():
    ...
    for r in rows:
        content+=f"<div class='card'><b>{h(r['approval_type'])} #{h(r['record_id'])}</b><br>Status: {h(r['status'])}<p>{h(r['notes'])}</p></div>"
    return render(content)

@app.route("/search")
def search():
    q=request.args.get("q","").strip()
    content=f"<div class='card hero'><h1>🔎 OAP Search</h1><p class='green'>OAP-first search. Not Google.</p></div><div class='card'><form method='GET'><input name='q' value='{h(q)}' placeholder='Search culture, artists, events, businesses, products, family, affiliate...'><button>Search</button></form></div>"
    if not q: return render(content)
    like=f"%{q}%"; results=[]; conn=db()
    queries=[
        ("Culture","SELECT title,category,body FROM culture_posts WHERE title LIKE ? OR body LIKE ? LIMIT 20",(like,like)),
        ("Artist","SELECT artist_name,genre,bio FROM artists WHERE artist_name LIKE ? OR genre LIKE ? OR bio LIKE ? LIMIT 20",(like,like,like)),
        ("Event","SELECT title,event_type,description FROM events WHERE title LIKE ? OR event_type LIKE ? OR description LIKE ? LIMIT 20",(like,like,like)),
        ("Business","SELECT business_name,category,description FROM businesses WHERE business_name LIKE ? OR category LIKE ? OR description LIKE ? LIMIT 20",(like,like,like)),
        ("Product","SELECT product_name,category,description FROM products WHERE product_name LIKE ? OR category LIKE ? OR description LIKE ? LIMIT 20",(like,like,like)),
        ("Family Tree","SELECT node_name,node_type,contribution_note FROM family_tree WHERE node_name LIKE ? OR node_type LIKE ? OR heritage_note LIKE ? OR contribution_note LIKE ? LIMIT 20",(like,like,like,like)),
        ("Affiliate Tree","SELECT affiliate_name,affiliate_type,proof_note FROM affiliate_tree WHERE affiliate_name LIKE ? OR affiliate_type LIKE ? OR referred_business LIKE ? OR proof_note LIKE ? LIMIT 20",(like,like,like,like)),
    ]
    for label, sql, params in queries:
        for r in conn.execute(sql, params):
            vals=list(r); results.append((label, vals[0], vals[1], vals[2]))
    conn.close()
...
@app.route("/explorer")
def explorer():
    return render("<div class='card hero'><h1>🧭 Explorer</h1></div><div class='grid'><div class='card'><h2>🌍 Community</h2><a href='/community'>Open</a></div><div class='card'><h2>🎵 Culture</h2><a href='/culture'>Open</a></div><div class='card'><h2>🎤 Artists</h2><a href='/artists'>Open</a></div><div class='card'><h2>📅 Events</h2><a href='/events'>Open</a></div><div class='card'><h2>🏪 Business</h2><a href='/business'>Open</a></div><div class='card'><h2>🛍 Products</h2><a href='/products'>Open</a></div><div class='card'><h2>🌳 Family Tree</h2><a href='/family_tree'>Open</a></div><div class='card'><h2>🌿 Affiliate Tree</h2><a href='/affiliate_tree'>Open</a></div></div>")
...
def command_center():
    ...
    commission=conn.execute("SELECT COALESCE(SUM(commission_value),0) c FROM affiliate_tree").fetchone()["c"]
    conn.close()
    return render(f"<div class='card hero'><h1>🎯 Command Center</h1></div><div class='grid'><div class='card'><h2>{members}</h2><p>Members</p></div><div class='card'><h2>{businesses}</h2><p>Businesses</p></div><div class='card'><h2>{products}</h2><p>Products</p></div><div class='card'><h2>{family}</h2><p>Family Tree</p></div><div class='card'><h2>{affiliate}</h2><p>Affiliate Tree</p></div><div class='card'><h2>£{total:.2f}</h2><p>Opportunity Board</p></div><div class='card'><h2>£{commission:.2f}</h2><p>Affiliate Commissions</p></div></div>")

@app.route("/health")
def health():
    conn=db(); checks=[]
    for table in ["users","audit_logs","culture_posts","artists","events","messages","trust_badges","opportunities","payouts","approvals","businesses","products","family_tree","affiliate_tree"]:
        try:
    ...
    conn.close()
    content="<div class='card hero'><h1>🧪 Health Check</h1><p class='green'>Accuracy before expansion.</p></div>"
