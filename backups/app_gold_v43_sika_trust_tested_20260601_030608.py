...
def sum_points(username=None):
    conn = db()
    try:
        if username:
            row = conn.execute("SELECT COALESCE(SUM(trust_points),0) s FROM sika_trust_records WHERE username=?", (username,)).fetchone()
        else:
            row = conn.execute("SELECT COALESCE(SUM(trust_points),0) s FROM sika_trust_records").fetchone()
        return row["s"]
    except Exception:
        return 0
    finally:
...
<a href="/sika_trust">SIKA Trust</a><a href="/hrm">HRM</a><a href="/menu">Menu</a>
...
def home():
    ...
    <p>Trust before finance. SIKA records value, not legal tender.</p>
    ...
    <div class='card'><h2>{sum_points()}</h2><p>Total SIKA Trust Points</p></div>
...
def my_world():
    ...
    <div class='card'><h2>{sum_points(u)}</h2><p>My SIKA Trust Points</p><a href='/sika_trust'>Open</a></div>
...
def menu():
    ...
    <div class='grid'>
    <div class='card'><h2>🌍 My World</h2><p><a href='/my_world'>My World</a></p><p><a href='/family_tree'>Family Tree</a></p><p><a href='/affiliate_tree'>Affiliate Tree</a></p><p><a href='/sika_trust'>SIKA Trust</a></p></div>
...
def sika_trust():
    ...
    <select name='record_type'>
      <option>Community Contribution</option><option>Business Value</option><option>Creator Value</option>
      <option>Event Support</option><option>Affiliate Proof</option><option>Family Tree Legacy</option>
      <option>Build Contribution</option><option>Trust Badge Proof</option>
    </select>
...
def search():
    ...
    content = f"<div class='card hero'><h1>🔎 OAP Search</h1></div><div class='card'><form><input name='q' value='{h(q)}' placeholder='Search OAP records'><button>Search</button></form></div>"
    if not q:
        return render(content)
    like = f"%{q}%"
    conn = db()
    rows = conn.execute("""SELECT record_title, record_type, value_note, trust_points FROM sika_trust_records
    WHERE record_title LIKE ? OR record_type LIKE ? OR value_note LIKE ? OR proof_note LIKE ? LIMIT 50""", (like,like,like,like)).fetchall()
    conn.close()
    content += "<div class='card'><h2>SIKA Results</h2>"
    for r in rows:
...
@app.route("/explorer")
def explorer():
    return render("<div class='card hero'><h1>🧭 Explorer</h1></div><div class='grid'><div class='card'><h2>My World</h2><a href='/my_world'>Open</a></div><div class='card'><h2>SIKA Trust</h2><a href='/sika_trust'>Open</a></div><div class='card'><h2>HRM</h2><a href='/hrm'>Open</a></div><div class='card'><h2>Sovereign Veto</h2><a href='/sovereign_veto'>Open</a></div></div>")
...
def command_center():
    ...
    <div class='grid'>
    <div class='card'><h2>{count("sika_trust_records")}</h2><p>SIKA Trust Records</p></div>
    <div class='card'><h2>{sum_points()}</h2><p>Total SIKA Points</p></div>
    <div class='card'><h2>{count("council_logs")}</h2><p>Council Logs</p></div>
...
@app.route("/health")
def health():
    tables = ["users","audit_logs","family_tree","affiliate_tree","sika_trust_records","council_logs","decision_logs","risk_logs","build_logs","sovereign_veto_logs"]
