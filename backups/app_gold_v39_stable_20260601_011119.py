...
def menu():
    ...
    <div class='grid'>
      ...
      <p><a href='/my_world'>My World</a></p><p><a href='/messages'>Messenger</a></p><p><a href='/trust_badge'>Trust Badge</a></p><p><a href='/payouts'>Payouts</a></p></div>
...
def payouts():
    if request.method=="POST":
        ...
        cur.execute("INSERT INTO payouts(username,recipient_name,reason,amount,currency,status,approved_by,paid_date,created_at) VALUES(?,?,?,?,?,?,?,?,?)",(request.form.get("username") or user(),request.form["recipient_name"],request.form["reason"],float(request.form.get("amount") or 0),request.form["currency"],"pending","","",now()))
        pid=cur.lastrowid
        cur.execute("INSERT INTO approvals(approval_type,record_id,status,reviewer,notes,created_at) VALUES(?,?,?,?,?,?)",("payout",pid,"pending","","Payout requires human approval.",now()))
...
def command_center():
    ...
    conn.close()
    return render(f"<div class='card hero'><h1>🎯 Command Center</h1></div><div class='grid'><div class='card'><h2>{members}</h2><p>Members</p></div><div class='card'><h2>{businesses}</h2><p>Businesses</p></div><div class='card'><h2>{products}</h2><p>Products</p></div><div class='card'><h2>£{total:.2f}</h2><p>Opportunity Board</p></div><div class='card'><h2>£{pending:.2f}</h2><p>Pending Payouts</p></div></div>")

@app.route("/health")
def health():
    conn=db()
    checks=[]
    for table in ["users","audit_logs","culture_posts","artists","events","messages","trust_badges","opportunities","payouts","approvals","businesses","products"]:
        try:
