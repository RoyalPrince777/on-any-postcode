...
def promo_card(p):
    html = f"<div class='card'><b>{p['title']}</b><br><span class='gold'>{p['category']}</span>"
    if p["image"]:
        html += f"<img src='/{p['image']}'>"
...
def news_card(n):
    ...
    <div class='card'>
    <b>{n['title']}</b><br>
    <span class='gold'>{n['category']}</span> • <span class='small'>{n['verification_status']}</span>
    """
    if n["image"]:
        html += f"<img src='/{n['image']}'>"
    html += f"<p>{n['summary'] or ''}</p><a href='/news/{n['id']}'>Read Story</a></div>"
    return html
...
def home():
    ...
    promos = conn.execute("SELECT * FROM promo_slots WHERE status='approved' ORDER BY id DESC LIMIT 3").fetchall()
    news = conn.execute("SELECT * FROM news_posts WHERE status='approved' ORDER BY id DESC LIMIT 4").fetchall()
    creators = conn.execute("SELECT * FROM creator_profiles ORDER BY id DESC LIMIT 4").fetchall()
    releases = conn.execute("SELECT * FROM media_releases ORDER BY id DESC LIMIT 4").fetchall()
    conn.close()
    ...
    <p>OAP = News + Community + Media + Retail + HRM</p>
    </div>
    """
    ...
    content += "<div class='card'><h2>📰 OAP News</h2><div class='grid'>"
    for n in news:
        content += news_card(n)
    content += "</div></div>"
...
def news():
    if request.method == "POST":
        if not session.get("user"):
            return redirect("/login")
        image = save_file(request.files.get("image"))
        related_product_id = request.form.get("related_product_id") or 0
        conn = db()
        ...
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            session["user"], request.form["title"], request.form["category"], request.form["summary"],
            request.form["body"], request.form["source_note"], request.form["verification_status"],
            image, related_product_id, request.form["related_link"], 0, "pending", now()
        ))
        conn.commit()
        conn.close()
        log("News story submitted", session["user"])
        return redirect("/news")

    conn = db()
    rows = conn.execute("SELECT * FROM news_posts ORDER BY id DESC").fetchall()
    products = conn.execute("SELECT id,title FROM products ORDER BY id DESC LIMIT 50").fetchall()
    conn.close()

    content = """
    <div class='card'><h1>📰 OAP News</h1>
    <p>Community, creators, retail, sports, business and verified local intelligence.</p>
    """
    if session.get("user"):
        content += """
        ...
        <select name='category'>
            <option>Community</option><option>Creator</option><option>Retail</option>
            <option>Sports</option><option>Business</option><option>Education</option>
            <option>Environment</option><option>Humanitarian-safe</option>
        </select>
        <textarea name='summary' placeholder='Short summary'></textarea>
        <textarea name='body' placeholder='Full story'></textarea>
        <textarea name='source_note' placeholder='Source / proof / verification note'></textarea>
        <select name='verification_status'>
            <option>draft</option><option>verified</option><option>needs review</option>
        </select>
        <input name='related_product_id' placeholder='Optional related product ID'>
        <input name='related_link' placeholder='Optional related link'>
        <input type='file' name='image'>
        <button>Submit News For HRM Review</button>
        </form>
        """
...
def news_detail(id):
    ...
    if n:
        ...
        conn.commit()
        if n["related_product_id"]:
            product = conn.execute("SELECT * FROM products WHERE id=?", (n["related_product_id"],)).fetchone()
    conn.close()
    if not n:
        return "News story not found"

    content = f"""
    <div class='card'>
    <h1>{n['title']}</h1>
    <p class='gold'>{n['category']} • {n['verification_status']}</p>
    <p class='small'>By @{n['username']} • {n['created_at']} • Views: {n['views']}</p>
    """
    if n["image"]:
        content += f"<img src='/{n['image']}'>"
    content += f"<h3>{n['summary'] or ''}</h3><p>{n['body'] or ''}</p>"
    content += f"<div class='card'><h2>Verification Note</h2><p>{n['source_note'] or ''}</p></div>"
    if product:
        content += "<div class='card'><h2>Related Product</h2>" + product_card(product) + "</div>"
    if n["related_link"]:
        content += f"<a class='badge' href='{n['related_link']}'>Open Related Link</a>"
    content += "</div>"
    return render(content)
...
def retail():
    if request.method == "POST":
        ...
        (username,title,category,price,supplier,sku,printful_id,printful_variant_id,description,image,product_link,status,stock_status,collection,views,clicks,created_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            session["user"], request.form["title"], request.form["category"], request.form["price"],
            request.form["supplier"], request.form["sku"], request.form["printful_id"],
            request.form["printful_variant_id"], request.form["description"], image,
            request.form["product_link"], "pending", request.form["stock_status"],
            request.form["collection"], 0, 0, now()
        ))
        conn.commit()
        conn.close()
    ...
    conn.close()
    content = """
    <div class='card'><h2>🛍️ OAP Retail + Printful Connector</h2>
    <p>OAP is the storefront. Printful/Printify/Tapstitch are fulfilment routes.</p>
    <form method='POST' enctype='multipart/form-data'>
    ...
    <select name='stock_status'>
        <option>available</option><option>limited</option><option>preorder</option><option>sold out</option>
    </select>
...
def product(id):
    ...
    <p>SKU: {p['sku'] or 'N/A'} | Printful ID: {p['printful_id'] or 'N/A'} | Variant: {p['printful_variant_id'] or 'N/A'}</p>
...
def order_intent():
    ...
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
    (
        request.form["product_id"], request.form["product_title"], username,
        request.form["customer_name"], request.form["customer_contact"], request.form["size"],
        request.form["colour"], request.form["quantity"], request.form["amount"],
        request.form["supplier"], request.form["fulfilment_link"], request.form["note"],
        "new", now()
    ))
    conn.commit()
    conn.close()
...
def orders():
    ...
    conn.close()
    content = "<div class='card'><h1>🧾 OAP Order Intents</h1></div>"
    for o in rows:
        content += f"""
        <div class='card'>
...
def promos():
    if request.method == "POST":
        ...
        VALUES(?,?,?,?,?,?,?,?,?,?)""",
        (session["user"], request.form["title"], request.form["category"], request.form["description"], request.form["link"], image, "pending", 0, 0, now()))
        conn.commit()
        conn.close()
...
def search():
    ...
    <form method='GET'>
    <input name='q' placeholder='Search news, products, creators, media, posts, promos...' value='""" + q + """'>
    ...
    like = f"%{q}%"
    conn = db()
    news = conn.execute("SELECT * FROM news_posts WHERE title LIKE ? OR category LIKE ? OR summary LIKE ? OR body LIKE ?", (like, like, like, like)).fetchall()
...
def retail_hrm():
    ...
    total_news = conn.execute("SELECT COUNT(*) c FROM news_posts").fetchone()["c"]
    ...
    news = conn.execute("SELECT * FROM news_posts ORDER BY views DESC LIMIT 20").fetchall()
    conn.close()
    content = f"""
    <div class='card'><h1>🧠 OAP Retail + News HRM</h1></div>
    <div class='grid'>
    ...
    <div class='card'><h2>{total_news}</h2><p>News Stories</p></div>
    </div>
    """
    ...
    content += "</div><div class='card'><h2>News Performance</h2>"
    for n in news:
        content += f"<div class='card'><b>{n['title']}</b> — {n['category']}<br>Views: {n['views']} | Status: {n['status']}</div>"
    content += "</div><div class='card'><h2>Promo Performance</h2>"
    for p in promos:
        content += f"<div class='card'><b>{p['title']}</b> — {p['status']}<br>Views: {p['views']} | Clicks: {p['clicks']}</div>"
    content += "</div><div class='card'><h2>Top Retail Products</h2>"
    for p in products:
    ...
    content += "</div><div class='card'><h2>Order Intents</h2>"
    for o in orders:
    ...
    content += "</div><div class='card'><h2>Retail Events</h2>"
    for e in events:
...
def admin():
    ...
    news = conn.execute("SELECT * FROM news_posts ORDER BY id DESC").fetchall()
    ...
    content += "<div class='card'><h2>News Review</h2>"
    for n in news:
        content += f"<div class='card'>{n['title']} — {n['status']} • {n['verification_status']} <a href='/admin/news/{n['id']}/approved'>Approve</a> <a href='/admin/news/{n['id']}/rejected'>Reject</a></div>"
    content += "</div><div class='card'><h2>Order Intents</h2>"
    for o in orders:
        content += f"<div class='card'>{o['product_title']} — {o['status']} — {o['amount']}<br>{o['customer_contact']}</div>"
    content += "</div><div class='card'><h2>Retail Products Review</h2>"
    for p in products:
    ...
    content += "</div><div class='card'><h2>Promo Review</h2>"
    for p in promos:
    ...
    content += "</div><div class='card'><h2>Media Releases Review</h2>"
    for r in releases:
...
def admin_news(id, status):
    ...
    conn.close()
    if n:
        log(f"News {status}: {n['title']}", session.get("user", "admin"))
    return redirect("/admin")
