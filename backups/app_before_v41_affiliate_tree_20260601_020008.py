...
def menu():
    ...
    <div class='grid'>
      ...
      <p><a href='/my_world'>My World</a></p><p><a href='/messages'>Messenger</a></p><p><a href='/trust_badge'>Trust Badge</a></p><p><a href='/family_tree'>Family Tree</a></p><p><a href='/payouts'>Payouts</a></p></div>
      ...
      <p><a href='/privacy'>Privacy & Trust</a></p><p><a href='/command_center'>Command Center</a></p><p><a href='/routes'>Routes</a></p><p><a href='/health'>Health Check</a></p><p><a href='/admin'>Admin</a></p></div>
    </div>
    """)
...
def family_tree():
    ...
    <div class='card warn'>Private-first. This is not a recruitment system. It records trust, heritage, mentorship, community and legacy.</div>
    <div class='card'><form method='POST'>
    ...
    """
    for r in rows:
        content+=f"<div class='card'><b>{h(r['node_name'])}</b><br>{h(r['node_type'])} → {h(r['connection_to'])}<br><span class='green'>{h(r['relationship'])}</span><p>{h(r['heritage_note'])}</p><p>{h(r['contribution_note'])}</p><p class='small'>{h(r['visibility'])} • {h(r['created_at'])}</p></div>"
...
@app.route("/approvals")
def approvals():
    return render("<div class='card hero'><h1>✅ Approvals</h1><p class='green'>Review before action.</p></div>")

@app.route("/search")
def search():
    q=request.args.get("q","").strip()
    content=f"<div class='card hero'><h1>🔎 OAP Search</h1><p class='green'>OAP-first search. Not Google.</p></div><div class='card'><form method='GET'><input name='q' value='{h(q)}' placeholder='Search culture, artists, events, businesses, products, family...'><button>Search</button></form></div>"
...
def command_center():
    ...
    conn.close()
    return render(f"<div class='card hero'><h1>🎯 Command Center</h1></div><div class='grid'><div class='card'><h2>{members}</h2><p>Members</p></div><div class='card'><h2>{businesses}</h2><p>Businesses</p></div><div class='card'><h2>{products}</h2><p>Products</p></div><div class='card'><h2>{family}</h2><p>Family Tree</p></div><div class='card'><h2>£{total:.2f}</h2><p>Opportunity Board</p></div></div>")

@app.route("/health")
def health():
    conn=db(); checks=[]
    for table in ["users","audit_logs","culture_posts","artists","events","messages","trust_badges","opportunities","payouts","approvals","businesses","products","family_tree"]:
        try:
