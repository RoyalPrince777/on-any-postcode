"""Governed external travel-data boundary for OAP.

OAP Travel does not register Booking.com or any other external marketplace as a
partner/provider. External information may be fetched on demand for research or
comparison, but it is not imported into OAP's public catalogue and never gains
booking, payment, supplier or authority status.

The provider contract remains available for a future separately approved direct
integration, but the active provider registry is intentionally empty.
"""
...
# Human Authority decision: no external travel marketplace is an OAP partner/provider.
...
def providers() -> tuple[SupplyProvider, ...]:
    """Return active commercial external provider declarations (currently none)."""

    return _PROVIDERS


def provider(provider_id: str) -> SupplyProvider | None:
    """Return one approved provider declaration, if one exists."""

    return _PROVIDER_BY_ID.get(str(provider_id or "").strip().casefold())


def normalize_external_observation(payload: Mapping[str, Any]) -> ExternalObservation:
    """Normalize research-only external data without creating supplier authority."""

    source_name = " ".join(str(payload.get("source_name") or "").strip().split())
    category = str(payload.get("category") or "").strip().casefold()
    title = " ".join(str(payload.get("title") or "").strip().split())
    place_label = " ".join(str(payload.get("place_label") or "").strip().split())
    observed_at = str(payload.get("observed_at") or "").strip()
    source_url = str(payload.get("source_url") or "").strip()
    if not source_name or not title or not place_label:
        raise ValueError("external_observation_identity_required")
...
def external_observation_record(payload: Mapping[str, Any]) -> dict[str, Any]:
    observation = normalize_external_observation(payload)
    return {
        **asdict(observation),
        "research_only": True,
        "catalogue_ingest_authorized": False,
...
def normalize_offer(payload: Mapping[str, Any]) -> SupplyOffer:
    """Validate an offer only for a separately registered commercial provider.

    The active registry is empty, so external marketplace offers fail closed and
    cannot silently become OAP catalogue inventory.
    """
...
def validate_supply_integration() -> dict[str, Any]:
    ...
    return {
        ...
        "external_catalogue_ingest_allowed": False,
...
def status() -> dict[str, Any]:
    ...
    return {
        "component": "OAP External Travel Data Boundary",
        ...
        "truth_boundary": (
            "OAP may fetch external travel information on demand for research and "
            "comparison. No external marketplace is registered as an OAP partner/provider, "
            "and fetched data is not OAP Direct inventory, booking authority or payment "
            "authority. Only directly onboarded Certified OAP Suppliers enter the public "
            "OAP Travel catalogue."
        ),
    }
