...
@bp.get("/api/sika/security/reauth-backoff")
@web_security.login_required(api=True)
def sika_security_reauth_backoff():
    owner = _authenticated_sika_owner()
    try:
        return jsonify(sika_security_ledger.reauth_backoff_state(owner.owner_id))
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "locked": True}), 503


@bp.post("/api/sika/security/step-up/proof")
@web_security.login_required(api=True)
def sika_security_step_up_proof():
    csrf_error = _csrf_or_403()
    if csrf_error:
        return csrf_error
    owner = _authenticated_sika_owner()
...
@bp.post("/api/sika/security/payment-review-gate")
@web_security.login_required(api=True)
def sika_security_payment_review_gate():
    csrf_error = _csrf_or_403()
    if csrf_error:
        return csrf_error
    owner = _authenticated_sika_owner()
    body = request.get_json(silent=True) or {}
    try:
        return jsonify(sika_security_ledger.final_payment_review_gate(
            owner.owner_id,
            challenge_id=body.get("challenge_id"),
            payment_intent_id=body.get("payment_intent_id"),
        ))
...
def sika_security_alert_dismiss():
    ...
    body = request.get_json(silent=True) or {}
    try:
        return jsonify(sika_security_ledger.dismiss_alert(
            owner.owner_id, body.get("security_event_id")
        ))
    except ValueError as exc:
...
def sika_security_sessions():
    owner = _authenticated_sika_owner()
    try:
        return jsonify(sika_security_ledger.session_registry(owner.owner_id))
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "sessions": [], "compromise_locked": True}), 503


@bp.post("/api/sika/security/session/register")
@web_security.login_required(api=True)
def sika_security_session_register():
    csrf_error = _csrf_or_403()
    if csrf_error:
        return csrf_error
    owner = _authenticated_sika_owner()
    body = request.get_json(silent=True) or {}
    try:
        result = sika_security_ledger.register_session(
            owner.owner_id,
            session_id=body.get("session_id"),
            device_id=body.get("device_id"),
        )
    except ValueError as exc:
        return jsonify({"error": str(exc), "registered": False}), 400
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "registered": False}), 503
    return jsonify(result), 201 if result.get("registered") else 200


@bp.post("/api/sika/security/session/trust")
@web_security.login_required(api=True)
def sika_security_session_trust():
    csrf_error = _csrf_or_403()
    if csrf_error:
        return csrf_error
    owner = _authenticated_sika_owner()
    body = request.get_json(silent=True) or {}
    try:
        return jsonify(sika_security_ledger.set_session_trust(
            owner.owner_id,
            session_id=body.get("session_id"),
            trusted=bool(body.get("trusted")),
        ))
    except ValueError as exc:
        return jsonify({"error": str(exc), "trusted": False}), 400
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "trusted": False}), 503
...
def sika_security_session_revoke():
    ...
    body = request.get_json(silent=True) or {}
    try:
        return jsonify(sika_security_ledger.revoke_session(
            owner.owner_id,
            session_id=body.get("session_id"),
        ))
    except ValueError as exc:
...
def sika_security_session_revoke_all():
    ...
    owner = _authenticated_sika_owner()
    try:
        return jsonify(sika_security_ledger.revoke_all_sessions(owner.owner_id))
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "revoked_all": False}), 503


@bp.post("/api/sika/security/session/compromise")
@web_security.login_required(api=True)
def sika_security_session_compromise():
    csrf_error = _csrf_or_403()
    if csrf_error:
        return csrf_error
    owner = _authenticated_sika_owner()
    body = request.get_json(silent=True) or {}
    try:
        return jsonify(sika_security_ledger.mark_session_compromised(
            owner.owner_id,
            session_id=body.get("session_id"),
        ))
    except ValueError as exc:
        return jsonify({"error": str(exc), "compromised": False}), 400
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "compromised": False}), 503
...
def sika_security_compromise_recover():
    ...
    owner = _authenticated_sika_owner()
    try:
        return jsonify(sika_security_ledger.recover_compromise(owner.owner_id))
    except sika_security_ledger.SikaSecurityLedgerUnavailable as exc:
        return jsonify({"error": str(exc), "recovered": False}), 503
