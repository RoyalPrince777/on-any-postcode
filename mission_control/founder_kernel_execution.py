        ...
        with postgres_db.connect() as connection:
            row = connection.execute(
                """SELECT 1 FROM audit_events
                   WHERE action='FOUNDER_TOOL_KERNEL_CLAIMED'
                     AND metadata->>'receipt_id'=%s
                   LIMIT 1""",
                (receipt.receipt_id,),
            ).fetchone()
            if row:
            ...
            approval_service._write_audit(
                ...
                reason="Living Kernel claimed exact Founder action receipt.",
    ...
    with postgres_db.connect(readonly=True) as connection:
        ...
        row = connection.execute(
            """SELECT receipt_id,request_id,identity_id,authority_level,decision,
                      issued_at,expires_at,nonce,action_digest,signature
...
def _plan_from_payload(payload: dict[str, Any]) -> ActionPlan:
    ...
    if not request_id or action_type not in _ALLOWED_ACTIONS or not isinstance(action_payload, dict):
...
def execute_approved_action(*, identity_id: str, receipt_id: str, plan_payload: dict[str, Any]) -> dict[str, object]:
    """Execute one exact Human-approved Founder action through canonical LivingKernel."""
    ...
    signing_key = approval_service._signing_key()
    identity = IdentityEngine((
        IdentityRecord(
            ...
            authority_level=0,
            status="ACTIVE",
            permissions=frozenset({HumanApprovalAuthority.APPROVAL_PERMISSION}),
            roles=("Founder",),
        ),
    ))
    approval = HumanApprovalAuthority(identity, signing_key)
    ...
    recommendation = Recommendation(
        ...
        war_room=WarRoomReport(
            triggered=False,
            scenarios=(),
            recommendation="Proceed only through Living Kernel after exact receipt verification.",
        ),
    )
...
def status() -> dict[str, object]:
    return {
        ...
        "ready": bool(os.getenv("OAP_GITHUB_TOKEN", "").strip() and os.getenv("OAP_APPROVAL_SIGNING_KEY", "").strip()),
