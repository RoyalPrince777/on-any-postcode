...
_CONTINENT_CODES = {
    "Africa": frozenset(
        ...
        "GN GW KE LS LR LY MG MW ML MR MU MA MZ NA NE NG RE RW SH ST SN SC SL "
        "SO ZA SS SD TZ TG TN UG EH ZM ZW YT".split()
    ),
    "Asia": frozenset(
        ...
        "TM AE UZ VN YE".split()
    ),
    "Europe": frozenset(
        "AX AL AD AT BY BE BA BG HR CZ DK EE FO FI FR DE GI GR GG HU IS IE IM "
        "IT JE LV LI LT LU MT MD MC ME NL MK NO PL PT RO RU SM RS SK SI ES SJ "
        "SE CH UA GB VA XK".split()
    ),
    "North America": frozenset(
        "AI AG AW BS BB BZ BM BQ CA KY CR CU CW DM DO SV GL GD GP GT HT HN JM "
        "MQ MX MS NI PA PR BL KN LC MF PM VC SX TT TC US VG VI".split()
    ),
    "South America": frozenset(
        "AR BO BR CL CO EC FK GF GY PY PE SR UY VE".split()
    ),
    "Oceania": frozenset(
        "AS AU CX CC CK FJ PF GU KI MH FM NR NC NZ NU NF MP PW PG PN WS SB TK "
        "TO TV UM VU WF".split()
    ),
