...
def smi_button_proof():
    """Return server proof plus observed successful Founder browser responses."""
...
def smi_button_click_receipt():
    """Record one successful visible Founder control response."""
    ...
    receipt = smi_receipt_backend.write_receipt(
        "smi_button_click_receipt",
        {
            ...
            "green_gate": "browser_observed_server_response_success",
    ...
    return _no_store(
        make_response(jsonify({"receipt": receipt, "recorded": bool(receipt.get("ok"))}))
    )
