...
def compose(location: Mapping[str, Any], weather: Mapping[str, Any]) -> dict[str, Any]:
    """Build a truthful Earth Intelligence view from already-verified inputs."""
    ...
    return {
        ...
        "local_conditions": {
            ...
            "condition": (
                weather_intelligence.get("condition") if weather_ready else "Conditions unavailable"
            ),
            "advisory_level": (
                weather_intelligence.get("advisory_level") if weather_ready else "unavailable"
            ),
...
def status(*, weather_ready: bool) -> dict[str, Any]:
    """Return architecture readiness without performing network I/O."""
