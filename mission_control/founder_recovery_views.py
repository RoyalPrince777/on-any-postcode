...
def _render(*, status_code: int = 200, error: str | None = None, next_path: str = _DEFAULT_NEXT, bind_mode: bool = False):
    response = make_response(render_template("founder_recovery.html", recovery_error=error, next_path=_safe_next(next_path), bind_mode=bind_mode), status_code)
    return _no_store(response)


def _render_bind(*, status_code: int = 200, error: str | None = None, next_path: str = _DEFAULT_NEXT, form_action: str | None = None, action_value: str = "bind-password", eyebrow: str | None = None, heading: str | None = None, subcopy: str | None = None, button_label: str | None = None, fine_print: str | None = None):
    response = make_response(render_template("founder_password_bind.html", bind_error=error, next_path=_safe_next(next_path), bind_form_action=form_action, bind_action_value=action_value, bind_eyebrow=eyebrow, bind_heading=heading, bind_subcopy=subcopy, bind_button_label=button_label, bind_fine_print=fine_print), status_code)
    return _no_store(response)
...
def _bind_existing_password(next_path: str):
    if not founder_recovery.session_active():
        return _render(status_code=403, error="Founder proof expired. Confirm the Founder recovery code again.", next_path=next_path, bind_mode=True)
    if not web_security.csrf_valid(request):
        return _render_bind(status_code=403, error="Session expired. Refresh and try again.", next_path=next_path)
    ...
    try:
        ...
        return _render_bind(status_code=503, error="Render Founder password storage is temporarily unavailable.", next_path=next_path)
    ...
    response.headers["X-OAP-Founder-Password-State"] = "rebound" if result == "rebound" else "bound"
    return _no_store(response)
...
def _normal_repair_configured(*, now: int | None = None) -> bool:
    ...
    return len(digest) == 64 and all(char in "0123456789abcdef" for char in digest) and _normal_repair_expiry() > current
...
def _begin_normal_repair_session(*, now: int | None = None) -> None:
    current = int(time.time()) if now is None else int(now)
    expires_at = min(current + _NORMAL_REPAIR_SESSION_SECONDS, _normal_repair_expiry())
    session[_NORMAL_REPAIR_SESSION_KEY] = {"version": 1, "token_hash": _normal_repair_hash(), "expires_at": expires_at}
...
def _normal_repair_session_active(*, now: int | None = None) -> bool:
    ...
    try:
        ...
        _clear_normal_repair_session()
        return False
    if expires_at <= current or expires_at > current + _NORMAL_REPAIR_SESSION_SECONDS or expires_at > _normal_repair_expiry():
        _clear_normal_repair_session()
        return False
    return hmac.compare_digest(str(value.get("token_hash") or ""), _normal_repair_hash())


def _render_normal_repair(*, status_code: int = 200, error: str | None = None, next_path: str = _DEFAULT_NEXT):
    return _render_bind(status_code=status_code, error=error, next_path=next_path, form_action=url_for("founder_recovery.repair_founder_password"), action_value="repair-normal-password", eyebrow="Private Founder Repair", heading="Restore normal Founder login", subcopy="Enter your original private password twice. OAP will replace only the salted Render-local verifier for the existing Human Authority identity.", button_label="Restore Normal Login", fine_print="No second Founder is created. After this repair, use Enter My World with your normal private password.")
...
def repair_founder_password():
    ...
    if request.method == "GET":
        ...
        if proof:
            ...
            return _no_store(redirect(url_for("founder_recovery.repair_founder_password", next=next_path)))
    ...
    if not _normal_repair_session_active():
        return _hidden()
    if not web_security.csrf_valid(request):
        return _render_normal_repair(status_code=403, error="Repair session expired. Open the repair link again.", next_path=next_path)
    if str(request.form.get("action") or "") != "repair-normal-password":
        return _hidden()
    password, error = _password_fields()
    if error:
        return _render_normal_repair(status_code=400, error=error, next_path=next_path)
    try:
        result = founder_local_auth.bind_existing_password(password)
    except founder_local_auth.FounderLocalAuthUnavailable:
        return _render_normal_repair(status_code=503, error="Render Founder password storage is temporarily unavailable.", next_path=next_path)
    if result not in {"bound", "rebound", "complete"}:
        return _render_normal_repair(status_code=503, error="Render Founder password storage is temporarily unavailable.", next_path=next_path)
    _clear_normal_repair_session()
    response = redirect(url_for("auth_page", next=next_path, render_bound="1"))
...
def recover_founder():
    ...
    if not web_security.csrf_valid(request):
        return _render(status_code=403, error="Session expired. Refresh and try again.", next_path=next_path, bind_mode=bind_mode)
    ...
    if not RECOVERY_BURST_LIMITER.allow(rate_key):
        response = _render(status_code=429, error="Founder recovery is temporarily protected. Wait briefly, then try once.", next_path=next_path, bind_mode=bind_mode)
        response.headers["Retry-After"] = "60"
        return response
    if not founder_recovery.token_allowed(request.form.get("recovery_code")):
        return _render(status_code=403, error="Code not recognised.", next_path=next_path, bind_mode=bind_mode)
