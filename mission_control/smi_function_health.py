...
def button_proof(url_map: Any) -> dict[str, Any]:
    """Return bounded server-side proof for Founder controls without faking browser clicks."""
    ...
    for item in health["functions"]:
        server_action_proven = bool(
            item["available"]
            and item["proof_checked"]
            and item["runtime_proven"]
        )
        rows.append(
            {
                "id": item["id"],
                ...
                "label": "SERVER PROVEN · CLICK PENDING" if server_action_proven else "PROOF REQUIRED",
                "consequential_execution": False,
            }
        )

    server_ready_count = sum(1 for row in rows if row["server_action_proven"])
    click_ready_count = sum(1 for row in rows if row["browser_click_proven"])
    ...
    return {
        ...
        "browser_click_ready_count": click_ready_count,
        "browser_click_ready_percent": _percent(click_ready_count, expected_count),
        "server_gate_green": server_ready_count == expected_count,
        "browser_click_gate_green": click_ready_count == expected_count,
        "whole_button_gate_green": bool(
            server_ready_count == expected_count
            and click_ready_count == expected_count
        ),
