...
function rich(body,text){body.textContent='';const blocks=String(text||'').split(/```/);blocks.forEach((block,index)=>{if(index%2){const first=block.indexOf('\n'),lang=first>=0?block.slice(0,first).trim():'code',code=first>=0?block.slice(first+1):block,wrap=document.createElement('div');wrap.className='code-block';const hd=document.createElement('div');hd.className='code-head';const label=document.createElement('span');label.textContent=lang||'code';const copy=document.createElement('button');copy.type='button';copy.className='copy';copy.textContent='Copy code';copy.onclick=async()=>{try{await navigator.clipboard.writeText(code);copy.textContent='Copied ✓'}catch{copy.textContent='Copy failed'}setTimeout(()=>copy.textContent='Copy code',1400)};hd.append(label,copy);const pre=document.createElement('pre'),c=document.createElement('code');c.textContent=code;pre.append(c);wrap.append(hd,pre);body.append(wrap);return}const lines=block.split('\n');let i=0;while(i<lines.length){const line=lines[i];if(!line.trim()){i++;continue}const h=line.match(/^(#{1,3})\s+(.+)$/);if(h){const e=document.createElement('div');e.className='md-h h'+h[1].length;inline(e,h[2]);body.append(e);i++;continue}if(/^>\s?/.test(line)){const e=document.createElement('div');e.className='md-quote';inline(e,line.replace(/^>\s?/,''));body.append(e);i++;continue}if(/^[-*]\s+/.test(line)){const ul=document.createElement('ul');ul.className='md-list';while(i<lines.length&&/^[-*]\s+/.test(lines[i])){const li=document.createElement('li');inline(li,lines[i].replace(/^[-*]\s+/,''));ul.append(li);i++}body.append(ul);continue}if(/^\d+\.\s+/.test(line)){const ol=document.createElement('ol');ol.className='md-list';while(i<lines.length&&/^\d+\.\s+/.test(lines[i])){const li=document.createElement('li');inline(li,lines[i].replace(/^\d+\.\s+/,''));ol.append(li);i++}body.append(ol);continue}if(i+1<lines.length&&line.includes('|')&&lines[i+1].includes('|')){
 ...
 if(headings.length>=2&&separators.length===headings.length&&separators.every(value=>/^:?-{3,}:?$/.test(value))){
  ...
  headings.forEach(value=>{const th=document.createElement('th');th.scope='col';inline(th,value);headRow.append(th)});
  thead.append(headRow);table.append(thead);
  const tbody=document.createElement('tbody');i+=2;
  while(i<lines.length&&lines[i].trim()&&lines[i].includes('|')){
   const values=cells(lines[i]),row=document.createElement('tr');
   headings.forEach((_,column)=>{const td=document.createElement('td');inline(td,values[column]||'');row.append(td)});
   tbody.append(row);i++;
  }
  table.append(tbody);scroll.append(table);body.append(scroll);continue;
 }
}const p=document.createElement('div');p.className='md-p';inline(p,line);body.append(p);i++}})}
...
const safeInspect=async(title,url,button)=>{if(!url)return;button.disabled=true;try{const r=await fetch(url,{cache:'no-store',credentials:'same-origin'}),d=await r.json();if(!r.ok)throw new Error(d?.error?.message||title+' unavailable');if(title==='Founder Library'){const assets=Array.isArray(d?.assets)?d.assets:[];const lines=d?.ready===false?['Founder Library unavailable · '+String(d?.error||'index pending')]:assets.length?assets.slice(0,12).map(asset=>String(asset.filename||'Saved asset').slice(0,120)+' · '+(asset.original_available===true?'Original availability must be authorised':'Fresh upload required')):['No indexed assets to show'];addTool('Founder Library · Saved Work',lines.join('\n')+'\nMetadata alone cannot supply original media. Re-upload for Studio uses your own file picker.','yellow');const result=messages?.lastElementChild;if(d?.ready!==false&&result){for(const asset of assets.slice(0,12)){if(asset.original_available===true)continue;const kind=String(asset.kind||'').toLowerCase();const selector=(kind==='image'||kind==='studio_image')?'#image-input':(['audio','video','document','studio_video'].includes(kind)?'#media-input':null);const action=document.createElement('button');action.type='button';action.textContent='📎 Re-upload '+String(asset.filename||'asset').slice(0,60)+' for Studio';action.setAttribute('aria-label','Choose a fresh '+(kind||'media')+' file for OAP Studio');action.onclick=()=>{if(!selector){addTool('Studio upload','Unsupported saved media type · no media sent','yellow');return;}const picker=q(selector);if(!picker){addTool('Studio upload','Compatible file picker unavailable · no media sent','yellow');return;}const studio=q('#studio-button');if(studio&&!studio.classList.contains('active'))addTool('Studio upload','After selecting your file, activate Studio for a governed creation request','purple');picker.value='';picker.click();};result.append(action);}}return;}const explicitFalse=d?.ready===false||d?.green===false||d?.schema_ready===false||d?.whole_smi_green===false;const explicitTrue=d?.ready===true||d?.green===true||d?.schema_ready===true||d?.whole_smi_green===true;addTool(title,JSON.stringify(d,null,2).slice(0,2200),explicitFalse&&!explicitTrue?'yellow':'green')}catch(e){addTool(title,e.message||'Unavailable','yellow')}finally{button.disabled=false;menu?.classList.remove('show')}};
