...
async function recordButtonSuccess(buttonId,path=''){
  if(!cfg.buttonClickReceiptUrl||!buttonId)return false;
  try{
    const r=await fetch(cfg.buttonClickReceiptUrl,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-OAP-CSRF':window.csrfToken||''},body:JSON.stringify({button_id:buttonId,response_class:'success',path})});
    return r.ok;
  }catch(_){return false}
}
const safeInspect=async(title,url,button,proofId='')=>{if(!url)return;button.disabled=true;try{const r=await fetch(url,{cache:'no-store',credentials:'same-origin'}),d=await r.json();if(!r.ok)throw new Error(d?.error?.message||title+' unavailable');const explicitFalse=d?.ready===false||d?.green===false||d?.schema_ready===false||d?.whole_smi_green===false||d?.whole_button_gate_green===false;const explicitTrue=d?.ready===true||d?.green===true||d?.schema_ready===true||d?.whole_smi_green===true||d?.whole_button_gate_green===true;addTool(title,JSON.stringify(d,null,2).slice(0,2600),explicitFalse&&!explicitTrue?'yellow':'green');if(proofId)await recordButtonSuccess(proofId,url)}catch(e){addTool(title,e.message||'Unavailable','yellow')}finally{button.disabled=false;menu?.classList.remove('show')}};
async function navigateProven(buttonId,url,button){
  if(!url)return;
  button.disabled=true;
  try{
    const r=await fetch(url,{cache:'no-store',credentials:'same-origin',redirect:'follow'});
    if(!r.ok)throw new Error('Destination returned HTTP '+r.status);
    await recordButtonSuccess(buttonId,url);
    location.href=url;
  }catch(e){addTool(buttonId,e.message||'Destination unavailable','yellow');button.disabled=false;menu?.classList.remove('show')}
}
const studioButton=q('#studio-button');if(studioButton)studioButton.onclick=async()=>{studioButton.disabled=true;try{const r=await fetch(cfg.studioStatusUrl,{cache:'no-store',credentials:'same-origin'}),d=await r.json();if(!r.ok)throw new Error(d?.error?.message||'Studio unavailable');studioMode=!studioMode;studioButton.classList.toggle('active',studioMode);const state=q('#studio-state'),backendConfigured=Boolean(d?.generation_backend_configured??d?.generation_backend_proven),fullLive=Boolean(d?.full_live_certificate);if(state)state.textContent=studioMode?(fullLive?'Live':backendConfigured?'Generate':'Plan'):'Off';if(studioMode){const tools=(d.generation_tools||[]).map(item=>item.name).join(' · ');addTool('OAP Studio Intelligence',[tools||'Studio tools available',backendConfigured?'Generation backend configured':'Generation backend unconfigured',fullLive?'Full live certificate proven':'Full live certificate pending artifact proof'].join('\n'),fullLive?'green':'yellow');q('#status').textContent=fullLive?'OAP Studio Intelligence live':'OAP Studio Intelligence active · SMI 21 governed generation';if(!input.value.trim()){input.placeholder='Create or analyse with OAP Studio Intelligence…';input.focus()}}else{q('#status').textContent='OAP Studio Intelligence off';input.placeholder='Ask SMI…'}}catch(e){q('#status').textContent=e.message||'Studio unavailable'}finally{studioButton.disabled=false;menu?.classList.remove('show')}};
...
async function runStudioTool(toolId,button){
  ...
  try{
    ...
    if(a.id){meta.textContent='🟣 Video job started · Chronicle '+(d.chronicle_receipt?.receipt_id||'recorded');await pollStudioVideo(a.id,card,meta);q('#status').textContent='Studio video complete · artifact proven';return}
...
qa('[data-oap-action]').forEach(button=>{button.onclick=()=>{const id=button.dataset.oapAction;
  if(id==='war-room'){navigateProven('war-room',cfg.warRoomUrl,button);return}
  if(id==='button-proof'){safeInspect('Button Proof',cfg.buttonProofUrl,button);return}
  if(id==='signals-21'){safeInspect('21 Signals',cfg.signalsUrl,button,'signals-21');return}
  if(id==='guardian'){safeInspect('Guardian',cfg.guardianUrl,button,'guardian');return}
  if(id==='routes'){safeInspect('Routes',cfg.routesUrl,button,'routes');return}
  if(id==='brain'){navigateProven('brain',cfg.brainUrl,button);return}
  if(id==='agents'){navigateProven('agents',cfg.agentsUrl,button);return}
  if(id==='infrastructure'){navigateProven('infrastructure',cfg.infrastructureUrl,button);return}
  if(id==='judgement'){navigateProven('judgement',cfg.judgementUrl,button);return}
  if(id==='improvement'){navigateProven('improvement',cfg.improvementUrl,button);return}
  if(id==='swot'){input.value=input.value.trim()?('Run SWOT Intelligence on this:\n'+input.value.trim()):'Run SWOT Intelligence on: ';input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();q('#status').textContent='SWOT Intelligence ready · Strengths · Weaknesses · Opportunities · Threats · Practical Move';menu?.classList.remove('show');return}
  if(id==='behaviour'){input.value=input.value.trim()?('Run Behaviour Intelligence on this:\n'+input.value.trim()):'Run Behaviour Intelligence on: ';input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();q('#status').textContent='Behaviour Intelligence ready · 21 dimensions · evidence-backed percentages only';menu?.classList.remove('show');return}
  if(id==='github-governed'){githubAction();return}
  if(id==='function-health'){safeInspect('Function Health',cfg.functionHealthUrl,button,'function-health');return}
  if(id==='green-gate'){safeInspect('Green Gate',cfg.greenGateUrl,button,'green-gate');return}
  if(id==='hrm'){safeInspect('HRM / Jog Memory',cfg.hrmUrl,button,'hrm');return}
  if(id==='founder-library'){safeInspect('Founder Library',cfg.founderLibraryUrl,button);return}
