...
qa('[data-oap-action]').forEach(button=>{button.onclick=()=>{const id=button.dataset.oapAction;if(id==='war-room'){recordButtonSuccess('war-room',cfg.warRoomUrl).finally(()=>location.href=cfg.warRoomUrl);return}if(id==='button-proof'){safeInspect('Button Proof',cfg.buttonProofUrl,button);return}if(id==='signals-21'){safeInspect('21 Signals',cfg.signalsUrl,button,'signals-21');return}if(id==='guardian'){safeInspect('Guardian',cfg.guardianUrl,button,'guardian');return}if(id==='routes'){safeInspect('Routes',cfg.routesUrl,button,'routes');return}if(id==='brain'){recordButtonSuccess('brain','/mission/brain').finally(()=>location.href='/mission/brain');return}if(id==='agents'){recordButtonSuccess('agents','/mission/agents').finally(()=>location.href='/mission/agents');return}if(id==='infrastructure'){recordButtonSuccess('infrastructure','/mission/infrastructure').finally(()=>location.href='/mission/infrastructure');return}if(id==='judgement'){recordButtonSuccess('judgement','/mission/judgement').finally(()=>location.href='/mission/judgement');return}if(id==='improvement'){location.href=cfg.improvementUrl;return}if(id==='swot'){input.value=input.value.trim()?('Run SWOT Intelligence on this:\n'+input.value.trim()):'Run SWOT Intelligence on: ';input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();q('#status').textContent='SWOT Intelligence ready · Strengths · Weaknesses · Opportunities · Threats · Practical Move';menu?.classList.remove('show');return}if(id==='behaviour'){input.value=input.value.trim()?('Run Behaviour Intelligence on this:\n'+input.value.trim()):'Run Behaviour Intelligence on: ';input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();q('#status').textContent='Behaviour Intelligence ready · 21 dimensions · evidence-backed percentages only';menu?.classList.remove('show');return}if(id==='github-governed'){githubAction();return}if(id==='function-health'){safeInspect('Function Health',cfg.functionHealthUrl,button,'function-health');return}if(id==='green-gate'){safeInspect('Green Gate',cfg.greenGateUrl,button,'green-gate');return}if(id==='hrm'){safeInspect('HRM / Jog Memory',cfg.hrmUrl,button,'hrm');return}if(id==='founder-library'){safeInspect('Founder Library',cfg.founderLibraryUrl,button);return}}});
