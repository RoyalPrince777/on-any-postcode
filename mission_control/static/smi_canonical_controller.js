...
let oapPaused=false,oapWorkStarted=0,oapWorkTimer=null,oapLiveConversation=false,oapSpeaking=false;
...
function oapSetLive(enabled){
 ...
 if(oapLiveConversation){oapVoiceEnabled=true;if(oapSpeaker){oapSpeaker.classList.add('active');oapSpeaker.setAttribute('aria-pressed','true');oapSpeaker.textContent='🔊 Voice reply';}oapSetStatus('Live SMI on · listening and speaking turn by turn');oapCharacterState('ready',{live:true});setTimeout(oapStartListening,180);}
 else{if(oapRecognition&&oapListening){try{oapRecognition.stop()}catch{}}oapSetStatus('Live SMI off');oapCharacterState('ready',{live:false});}
}
function oapStartListening(){
 if(!oapLiveConversation||oapLocked||oapListening||oapSpeaking||!oapRecognition)return;
 try{oapSetStatus('Live SMI · microphone permission required…');oapRecognition.start();}catch{oapSetStatus('Live SMI microphone unavailable');oapSetLive(false);}
}
...
function oapBeginWork(){oapWorkStarted=performance.now();oapCharacterState('thinking');oapUpdateWorkedFor();if(oapWorkTimer)clearInterval(oapWorkTimer);oapWorkTimer=setInterval(oapUpdateWorkedFor,100);}
...
function oapSpeak(text){
 if(!text){oapCharacterState('ready');if(oapLiveConversation)setTimeout(oapStartListening,250);return;}
 if(!oapVoiceEnabled||!('speechSynthesis' in window)){oapCharacterState('ready');if(oapLiveConversation)setTimeout(oapStartListening,250);return;}
 window.speechSynthesis.cancel();
 const utterance=new SpeechSynthesisUtterance(String(text));utterance.lang='en-GB';
 utterance.onstart=()=>{oapSpeaking=true;oapCharacterState('speaking');};
 utterance.onend=()=>{oapSpeaking=false;oapCharacterState('ready');if(oapLiveConversation)setTimeout(oapStartListening,320);};
 utterance.onerror=()=>{oapSpeaking=false;oapCharacterState('ready');if(oapLiveConversation)setTimeout(oapStartListening,320);};
...
function oapStopAll(){responseStopped=true;oapPaused=false;oapSetLive(false);oapCharacterState('stopped');if(oapAbort)oapAbort.abort();if(oapRecognition&&oapListening){try{oapRecognition.stop()}catch{}}oapStopListenTimer();if(oapWorkTimer)clearInterval(oapWorkTimer);oapWorkTimer=null;oapWorkStarted=0;if('speechSynthesis' in window)window.speechSynthesis.cancel();oapRelease();try{hideThinking()}catch{}try{setRunning(false)}catch{}oapSetStatus('Response stopped by Human Authority');}
function oapTogglePause(){if(!oapLocked)return;oapPaused=!oapPaused;oapCharacterState(oapPaused?'paused':'thinking');if(oapPause){oapPause.classList.toggle('active',oapPaused);oapPause.setAttribute('aria-pressed',String(oapPaused));oapPause.textContent=oapPaused?'▶':'Ⅱ';}const provider=document.getElementById('provider-state');if(provider)provider.textContent=oapPaused?'Paused':'Working';if('speechSynthesis' in window){if(oapPaused)window.speechSynthesis.pause();else window.speechSynthesis.resume();}oapSetStatus(oapPaused?'Paused by Human Authority':'Resumed');}
...
function oapAddCaptureOptions(){if(!oapAttachMenu||oapAttachMenu.dataset.oapCaptureReady==='true')return;oapAttachMenu.dataset.oapCaptureReady='true';const camera=document.createElement('button');camera.type='button';camera.className='attach-option';camera.id='camera-button';camera.textContent='📷 Camera';camera.setAttribute('aria-label','Capture camera image');camera.addEventListener('click',oapCamera);const screen=document.createElement('button');screen.type='button';screen.className='attach-option';screen.id='screen-button';screen.textContent='🖥️ Share Screen';screen.setAttribute('aria-label','Share screen and capture frame');screen.addEventListener('click',oapScreen);oapAttachMenu.prepend(screen);oapAttachMenu.prepend(camera);}
...
async function oapSubmit(){
 ...
 oapLocked=true;responseStopped=false;oapPaused=false;oapInput.value='';oapSetStatus('Command received · generating governed result');oapAbort=new AbortController();activeController=oapAbort;setRunning(true);oapBeginWork();showStage('Understand');showStage('Context');let assistantBody=null,completeResult=null,streamError=null,streamText='';
 try{
...
if(oapMic){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(Recognition){oapRecognition=new Recognition();oapRecognition.lang='en-GB';oapRecognition.interimResults=true;oapRecognition.continuous=false;oapRecognition.onstart=()=>{oapListening=true;oapCharacterState('listening');oapListenStarted=Date.now();oapMic.classList.add('active');oapMic.setAttribute('aria-pressed','true');oapMic.setAttribute('aria-label','Stop voice input');oapMic.textContent='■';oapSetStatus('Listening · 0s');oapListenTimer=setInterval(()=>oapSetStatus(`Listening · ${oapElapsed()}s`),1000);};oapRecognition.onresult=event=>{oapInput.value=Array.from(event.results).map(result=>result[0].transcript).join('');oapInput.dispatchEvent(new Event('input',{bubbles:true}));};oapRecognition.onend=()=>{oapStopListenTimer();oapListening=false;oapMic.classList.remove('active');oapMic.setAttribute('aria-pressed','false');oapMic.setAttribute('aria-label','Voice input');oapMic.textContent='🎙️';oapCharacterState('ready');if(oapLiveConversation){if(oapInput.value.trim())setTimeout(oapSubmit,120);else setTimeout(oapStartListening,350);}else{oapSetStatus('Voice captured · edit or send');}};oapRecognition.onerror=event=>{oapStopListenTimer();oapListening=false;oapMic.classList.remove('active');oapMic.setAttribute('aria-pressed','false');oapMic.setAttribute('aria-label','Voice input');oapMic.textContent='🎙️';oapCharacterState('ready');if(oapLiveConversation)oapSetLive(false);oapSetStatus(event?.error==='not-allowed'?'Microphone permission blocked':'Voice input unavailable');};oapMic.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();try{if(oapListening)oapRecognition.stop();else{oapSetStatus('Microphone permission required…');oapRecognition.start();}}catch{oapSetStatus('Voice input unavailable');}},true);}else{oapMic.disabled=true;oapMic.title='Voice input is not supported by this browser';oapMic.setAttribute('aria-disabled','true');}}
if(oapLiveToggle)oapLiveToggle.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();oapSetLive(!oapLiveConversation);},true);
oapCharacterState('ready',{live:false});
oapAddCaptureOptions();
window.OAP_SMI_CANONICAL={version:'1.4',singleSubmitOwner:true,composerOwner:true,plusOwner:true,pauseOwner:true,timingOwner:true,thinkingModeOwner:true,studioModeOwner:true,micOwner:true,voiceOwner:true,stopOwner:true,cameraCapture:true,screenCapture:true,studioDuplicate:false,liveCharacter:true,halfDuplexLiveVoice:true,resultStreamOnly:true};
})();
