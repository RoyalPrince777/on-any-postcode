...
function oapSetLive(enabled){
 ...
 oapClearLiveRestart();
 oapSpeechSeq+=1;
 oapApply('LIVE_OFF');oapProof('liveOff',{epoch:oapRuntime?.epoch});
...
function oapSpeak(text){
 ...
 utterance.onstart=()=>{if(seq!==oapSpeechSeq||!oapStateApi.tokenIsCurrent(oapRuntime,expected)){oapProof('staleCallbackSuppressed',{source:'tts-start'});return;}oapApply('SPEAK_START');oapProof('speakStart',{epoch:oapRuntime?.epoch});};
 const finish=()=>{if(seq!==oapSpeechSeq||!oapStateApi.tokenIsCurrent(oapRuntime,expected)){oapProof('staleCallbackSuppressed',{source:'tts-finish'});return;}oapApply('SPEAK_END');oapProof('speakEnd',{epoch:oapRuntime?.epoch});if(oapRuntime.live)oapScheduleListening(320);};
...
function oapStopAll(){
 ...
 oapClearLiveRestart();
 oapSpeechSeq+=1;
 oapApply('STOP');oapProof('stop',{epoch:oapRuntime?.epoch});
 oapUpdateLiveToggle();
...
async function oapSubmit(options={}){
 ...
 try{
  ...
  while(true){
   ...
   const chunk=await reader.read();if(!ownsRequest())return;if(chunk.done)break;
   buffer=(buffer+decoder.decode(chunk.value,{stream:true})).replace(/\r\n/g,'\n');
   let boundary=-1;
   while((boundary=buffer.indexOf('\n\n'))>=0){
    if(!ownsRequest())return;const block=buffer.slice(0,boundary);buffer=buffer.slice(boundary+2);const parsed=parseEventBlock(block);if(!parsed)continue;
 ...
 }catch(error){if(ownsRequest()&&error?.name!=='AbortError'){add(error?.message||'Request not completed safely','system');oapSetStatus('Request not completed safely');}}
