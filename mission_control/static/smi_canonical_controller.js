...
function oapProofSnapshot(){
 return JSON.parse(JSON.stringify({
  ...
  localReplyAudio:oapLocalPlayer?.snapshot?.()||null,
 }));
}
...
function oapSpeak(text,recordedReply=null,explicit=false){
 if(oapRuntime?.stopped)return;
 if(!text||(!oapVoiceEnabled&&!explicit)){if(oapRuntime?.live)oapScheduleListening(250);return;}
 oapSpeechSeq+=1;
...
async function oapSubmit(options={}){
 ...
 const selectedStudioWorkspace=String(window.OAP_SMI_STUDIO_WORKSPACE||'auto');
...
if(oapMic){
 ...
 if(Recognition&&oapStateApi){
  ...
  oapMic.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();if(oapVoiceEnabled)oapLocalPlayer?.prepare?.();if(oapRuntime?.listening){try{oapRecognition.stop()}catch{}}else{oapRequestListening('manual');}},true);
 }else{
...
document.addEventListener('visibilitychange',()=>{
 ...
 if(oapRuntime?.speaking)oapApply('SPEAK_END');
 oapPlaybackState('cancelled',oapRuntime?.epoch);
 const motion=window.OAP_SMI_SOURCE_PIXEL_MOTION_SESSION?.snapshot?.()||null,audio=oapLocalPlayer?.snapshot?.()||null;
 window.dispatchEvent(new CustomEvent('oap-smi-background-stop',{detail:{audioStopped:Boolean(audio&&!audio.active),motionStopped:Boolean(motion&&!motion.localAudio&&!motion.live),epoch:oapRuntime?.epoch,atMs:performance.now()}}));
});
...
window.OAP_SMI_READ_ALOUD=text=>{
 ...
 if(typeof text!=='string'||!text.trim()){oapSetStatus('Read aloud unavailable');return false;}
