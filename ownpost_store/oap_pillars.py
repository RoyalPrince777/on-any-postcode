...
PILLARS = {
 ...
 'readiness_audit':['/api/checkpoints','/api/readiness/capabilities'],
}
...
def register_pillars(app):
 ...
 def pillar_status():
  ...
  return jsonify(ok=True,pillars=rows,internal_overall='green' if all(x['status']=='green' for x in rows) else 'red',green_definition='all_internal_pillar_probes_pass',external_dependencies_separate=True,no_fake_green=True,authority='human_final',checked_at=int(time.time()))
