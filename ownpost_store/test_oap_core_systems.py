...
class OAPCoreSystems(unittest.TestCase):
 ...
 def test_smi_awareness_and_controlled_self_improvement(self):
  s=self.c.get('/api/smi/self'); self.assertEqual(s.status_code,200); self.assertEqual(s.json['awareness_type'],'operational_machine_self_awareness'); self.assertEqual(s.json['probe_graph'],'leaf_only_acyclic'); self.assertFalse(s.json['phenomenal_consciousness_claim']); self.assertEqual(s.json['authority'],'human_final')
