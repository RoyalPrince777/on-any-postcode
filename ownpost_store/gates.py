...
from signal_trust import register_signal_trust
...
if 'oap_intelligence' not in app.blueprints: register_oap_intelligence(app, db, uid)
if 'youth_real_education' not in app.blueprints: register_youth_real_education(app, db, uid)
if 'oap_youth_club' not in app.blueprints: register_youth_club(app, db, uid)
if 'bank_intelligence' not in app.blueprints: register_bank_intelligence(app, db, uid)
if 'background_258' not in app.blueprints: register_background_258(app, db, uid)
if 'oap_ride' not in app.blueprints: register_oap_ride(app, db, uid)
if 'oap_ride_admin' not in app.blueprints: register_oap_ride_admin(app, db, uid)
if 'oap_movement_hub' not in app.blueprints: register_movement_hub(app, db, uid)
if 'oap_language' not in app.blueprints: register_oap_language(app, db, uid)
if 'oap_signal_pulse_controls' not in app.blueprints: register_signal_pulse_controls(app, db, uid)
if 'oap_signal_trust' not in app.blueprints: register_signal_trust(app, db, uid)
if 'oap_signal_intelligence' not in app.blueprints: register_signal_intelligence(app, db)
if 'oap_checkpoints' not in app.blueprints: register_checkpoints(app, db, uid)
if 'oap_provider_contracts' not in app.blueprints: register_provider_contracts(app)
if 'oap_provider_adapters' not in app.blueprints: register_provider_adapters(app)
if 'oap_location_bridge' not in app.blueprints: register_location_bridge(app)
